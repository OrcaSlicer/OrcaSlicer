...
StringObjectException Print::sequential_print_clearance_valid(const Print &print, Polygons *polygons, std::vector<std::pair<Polygon, float>>* height_polygons)
{
    ...
    std::vector<struct print_instance_info> print_instance_with_bounding_box;
    {
        ...
        // BBS: Add global skirt and per-object brim expansion for sequential print collision detection.
        // object_skirt_offset already covers per-object skirts (stPerObject); this handles global skirt
        // distance and brim_width for cases where they extend beyond the clearance radius envelope.
        {
...
void Print::update_filament_maps_to_config(std::vector<int> f_maps, std::vector<int> f_volume_maps, std::vector<int> f_nozzle_maps)
{
    if ((m_config.filament_map.values != f_maps) || (m_config.filament_volume_map.values != f_volume_maps) || (m_config.filament_nozzle_map.values != f_nozzle_maps))
    {
        ...
        {
            int safe_extruder_id_1based = f_maps[index];
            ExtruderType extruder_type = (ExtruderType)(opt_extruder_type->get_at(safe_extruder_id_1based - 1));
            NozzleVolumeType nozzle_volume_type = (NozzleVolumeType)(opt_nozzle_volume_type->get_at(safe_extruder_id_1based - 1));
            if (f_volume_maps.empty()) {
            ...
            m_config.filament_map_2.values[index] = m_ori_full_print_config.get_index_for_extruder(safe_extruder_id_1based, "print_extruder_id", extruder_type, nozzle_volume_type, "print_extruder_variant");
        }
...
void Print::_make_wipe_tower()
{
    ...
    if (!is_wipe_tower_type2) {
        ...
        // H2C: plan_toolchange queries the nozzle group result via
        ...
        for (auto& layer_tools : m_wipe_tower_data.tool_ordering.layer_tools()) { // for all layers
            ...
            for (const auto filament_id : layer_tools.extruders) {
                ...
                int extruder_id = 0;
                int nozzle_id = 0;
                int prev_nozzle_filament = -1;

                if (group_result) {
                    auto nozzle_info = group_result->get_nozzle_for_filament(filament_id, layer_idx);
                    if (nozzle_info) {
                        extruder_id = nozzle_info->extruder_id;
                        nozzle_id = nozzle_info->group_id;
                        prev_nozzle_filament = nozzle_recorder.get_filament_in_nozzle(nozzle_id);
                    }
                }
                ...
                bool nozzle_already_loaded = group_result && (prev_nozzle_filament == (int)filament_id);

                float volume_to_purge = 0;

                if (!nozzle_recorder.is_nozzle_empty(nozzle_id) && (int)filament_id != prev_nozzle_filament) {
                    // Ensure indices are within bounds of the flush_matrix
                    if (extruder_id < (int)multi_extruder_flush.size() &&
                        prev_nozzle_filament < (int)multi_extruder_flush[extruder_id].size() &&
                        filament_id < (int)multi_extruder_flush[extruder_id][prev_nozzle_filament].size())
                    {
                ...
                // H2C BBL: Use per-filament prime volumes with EC/NC distinction
                float wipe_volume_ec = m_config.filament_prime_volume.get_at(filament_id);
                float wipe_volume_nc = m_config.filament_prime_volume_nc.get_at(filament_id);
                // special prime volume settings for H2C saving mode
