...
StringObjectException Print::validate(std::vector<StringObjectException> *warnings, Polygons* collison_polygons, std::vector<std::pair<Polygon, float>>* height_polygons) const
{
    ...
    // Backstop for the GUI gate in ConfigManipulation: that dialog only fires on an edit, so a
    // config arriving by 3MF, imported preset or hand-edit can still carry the combination.
    // Hard error rather than a warning -- a zone with no shell is not degraded output, it is a
    // channel with nothing holding the injected melt in.
    for (PrintObject *object : m_objects) {
        if (object->config().wall_generator.value == PerimeterGeneratorType::Arachne)
            continue;
        for (const auto &region : object->all_regions()) {
            if (! region.get().config().dual_infill_enabled.value)
                continue;
            {
                return { L("Dual infill zones require the Arachne wall generator.\n\n"
                           "The zone shell is generated by Arachne only. With the classic generator "
                           "the zone still gets its floor, ceiling and interior, but no shell wall "
                           "-- so there is nothing containing the injected material laterally.\n\n"
                           "Set Quality > Wall generator to Arachne, or turn off Dual infill "
                           "zones."),
                         object, "wall_generator" };
            }
        }
    }
        ...
        for (const auto& region : object->all_regions()) {
            ...
            // (MagmaTriangleCell.hpp) so the warning can't drift.
            {
                ...
                if (m_config.gcode_flavor.value == gcfKlipper &&
                    ...
                    warn(L("Magma plunge injection will abort on stock Klipper: it folds a large "
                ...
                if (m.pattern == ipMagmaTriangle) {
                    warn(L("Magma Triangle has the least efficient seal geometry of the Magma patterns. "
                         ...
                         "dual_infill_outer_pattern", object);
                }
            }
            ...
            // silently capped at G-code time, so warn the user it won't run as set.
            {
                ...
                if (max_vol > 0.0 && inj_speed > max_vol) {
                    warn(Slic3r::format(
...
void Print::process(long long *time_cost_with_cache, bool use_cache)
{
    ...
    if (this->set_started(psMagmaInjectionOrder)) {
        ...
        if (first_magma != nullptr) {
            ...
            // travel_speed is per-nozzle since upstream's refactor. This is the spread-heat
            // timing model, which is object-wide rather than per-tool, so it reads the extruder
            // that performs the injections -- the tool whose travel actually paces the clock.
            magma::MagmaResolved tm;
            const int timing_ext = (! first_magma->all_regions().empty() &&
                                    magma::resolve_magma(first_magma->all_regions().front().get().config(),
                                                         oc, m_config, tm))
                                       ? tm.injection_extruder : 0;
            double travel = m_config.travel_speed.get_at(timing_ext);
