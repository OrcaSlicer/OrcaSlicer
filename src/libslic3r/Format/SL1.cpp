...
static const constexpr char* SL1_CONFIG_FNAME  = "config.ini";
static const constexpr char* SL1_PROFILE_FNAME = "prusaslicer.ini";

boost::property_tree::ptree sl1_read_ini(const mz_zip_archive_file_stat& entry, MZ_Archive& zip)
{
...
ArchiveData extract_sla_archive(const std::string& zipfname, const std::string& exclude)
{
    ...
    for (mz_uint i = 0; i < num_entries; ++i) {
        ...
        if (mz_zip_reader_file_stat(&zip.arch, i, &entry)) {
            ...
            if (name == SL1_CONFIG_FNAME)
                arch.config = sl1_read_ini(entry, zip);
            if (name == SL1_PROFILE_FNAME)
                arch.profile = sl1_read_ini(entry, zip);
