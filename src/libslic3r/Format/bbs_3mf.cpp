...
static bool is_path_within_root(const std::string& file_path, const boost::filesystem::path& root)
{
    ...
    try {
        ...
        if (full_str.length() > root_str.length() && full_str[root_str.length()] != boost::filesystem::path::preferred_separator)
            return false;
...
template<typename T> void add_vector(std::stringstream& stream, const std::vector<T>& values)
{
...
void PlateData::parse_filament_info(GCodeProcessorResult* result)
{
    ...
    auto get_used_filament_from_volume      = [m_filament_diameters, m_filament_densities](double volume, int extruder_id) {
        ...
        std::pair<double, double> ret = {section_area < EPSILON ? 0 : (koef * volume / section_area),
...
private:
    ...
    bool _create_object_instance(std::string const& path,
                                 int object_id,
                                 const Transform3d& transform,
                                 const bool printable,
                                 const bool auto_drop,
    ...
    for (mz_uint i = 0; i < num_entries; ++i) {
        if (mz_zip_reader_file_stat(&archive, i, &stat)) {
            ...
            if (name.find("/../") != std::string::npos || name.find("../") == 0 ||
                (name.length() >= 3 && name.compare(name.length() - 3, 3, "/..") == 0) || name == "..") {
                BOOST_LOG_TRIVIAL(warning) << __FUNCTION__
                                           << boost::format(", find file path including path traversal, not valid, skip it\n");
                continue;
            }
...
void _BBS_3MF_Importer::_extract_auxiliary_file_from_archive(mz_zip_archive& archive, const mz_zip_archive_file_stat& stat, Model& model)
{
    ...
    if (stat.m_uncomp_size > 0) {
        ...
        if (!is_path_within_root(dest_file, dir)) {
            BOOST_LOG_TRIVIAL(warning) << __FUNCTION__
                                       << boost::format(", path traversal detected in auxiliary file: %1%, skipping") % dest_file;
            return;
        }
...
static void to_xml(
    std::stringstream& stream, const EmbossShape& es, const ModelVolume& volume, mz_zip_archive& archive, bool export_full_path);
...
bool _BBS_3MF_Importer::_create_object_instance(std::string const& path,
                                                int object_id,
                                                const Transform3d& transform,
                                                const bool printable,
                                                const bool auto_drop,
...
class _BBS_3MF_Exporter : public _BBS_3MF_Base
{
    struct BuildItem
    {
        ...
        BuildItem(std::string const& path, unsigned int id, const Transform3d& transform, const bool printable, const bool auto_drop)
            : path(path), id(id), transform(transform), printable(printable), auto_drop(auto_drop)
        {}
    };
                                            ...
                                            int export_plate_idx)
{
    ...
    if (!_add_model_config_file_to_archive(archive, model, plate_data_list, objects_data, *config, export_plate_idx, m_save_gcode,
                                           m_use_loaded_id)) {
    ...
    for (unsigned int i = 0; i < (unsigned int) plate_data_list.size(); ++i) {
        ...
        if (plate_data != nullptr) {
            ...
            if (filament_map_mode_opt != nullptr && filament_map_mode_names.size() > filament_map_mode_opt->getInt())
                stream << "    <" << METADATA_TAG << " " << KEY_ATTR << "=\"" << FILAMENT_MAP_MODE_ATTR << "\" " << VALUE_ATTR << "=\""
                                                               ...
                                                               const DynamicPrintConfig& config)
{
    ...
    stream << "    <" << SLICE_HEADER_ITEM_TAG << " " << KEY_ATTR << "=\"" << "X-BBL-Client-Version" << "\" " << VALUE_ATTR << "=\""
           << convert_to_full_version(SLIC3R_VERSION) << "\"/>\n";
    stream << "    <" << SLICE_HEADER_ITEM_TAG << " " << KEY_ATTR << "=\"" << "OrcaSlicer-Version" << "\" " << VALUE_ATTR << "=\""
           << SoftFever_VERSION << "\"/>\n";
    stream << "  </" << SLICE_HEADER_TAG << ">\n";
    ...
    for (unsigned int i = 0; i < (unsigned int) plate_data_list.size(); ++i) {
        ...
        if (plate_data != nullptr && plate_data->is_sliced_valid) {
            ...
            const bool has_multi_nozzle_extruder = extruder_max_nozzle_count_option &&
                                                   std::any_of(extruder_max_nozzle_count_option->values.begin(),
                                                               extruder_max_nozzle_count_option->values.end(), [](int v) { return v > 1; });
            ...
            stream << "    <" << METADATA_TAG << " " << KEY_ATTR << "=\"" << FIRST_LAYER_TIME_ATTR << "\" " << VALUE_ATTR << "=\""
                   << plate_data->first_layer_time << "\"/>\n";
            ...
            if (plate_data && plate_data->nozzle_group_result)
                stream << "    <" << METADATA_TAG << " " << KEY_ATTR << "=\"" << ENABLE_FILAMENT_DYNAMIC_MAP_ATTR << "\" " << VALUE_ATTR
                       << "=\"" << std::boolalpha << plate_data->nozzle_group_result->is_support_dynamic_nozzle_map() << "\"/>\n";
            else
                stream << "    <" << METADATA_TAG << " " << KEY_ATTR << "=\"" << ENABLE_FILAMENT_DYNAMIC_MAP_ATTR << "\" " << VALUE_ATTR
                       << "=\"" << std::boolalpha << false << "\"/>\n";
            {
                bool has_filament_switcher = config.has("has_filament_switcher") ? config.opt_bool("has_filament_switcher") : false;
                stream << "    <" << METADATA_TAG << " " << KEY_ATTR << "=\"" << HAS_FILAMENT_SWITCHER_ATTR << "\" " << VALUE_ATTR << "=\""
                       << std::boolalpha << has_filament_switcher << "\"/>\n";
            }
            ...
            for (auto it = plate_data->objects_and_instances.begin(); it != plate_data->objects_and_instances.end(); it++) {
                ...
                if (use_gcode_object_name)
                    ...
                    object_name = sanitize_object_label(sanitize_object_label(obj->name) + "_id_" + std::to_string(gcode_object_index) +
                                                        "_copy_" + std::to_string(gcode_copy_index));

                stream << "    <" << OBJECT_TAG << " " << IDENTIFYID_ATTR << "=\"" << std::to_string(identify_id) << "\" " << NAME_ATTR
                       << "=\"" << xml_escape(object_name) << "\" " << SKIPPED_ATTR << "=\"" << (skipped ? "true" : "false") << "\" />\n";
            }
            ...
            for (auto it = plate_data->slice_filaments_info.begin(); it != plate_data->slice_filaments_info.end(); it++) {
                ...
                const std::string filament_nozzle_group_id = it->group_id.empty() ? std::to_string(nozzle_group_id) :
                                                                                    join_int_list_comma(it->group_id);
                ...
                const double filament_nozzle_diameter         = (has_multi_nozzle_extruder && it->nozzle_diameter > 0.0) ?
                                                                    it->nozzle_diameter :
                                                                    get_nozzle_diameter(nozzle_group_id);
                const std::string filament_nozzle_volume_type = it->nozzle_volume_type.empty() ? get_nozzle_volume_type(nozzle_group_id) :
                                                                                                 it->nozzle_volume_type;
                ...
                stream << "    <" << FILAMENT_TAG << " " << FILAMENT_ID_TAG << "=\"" << std::to_string(it->id + 1) << "\" "
                       ...
                       << FILAMENT_USED_G_TAG << "=\"" << it->used_g << "\" " << FILAMENT_NOZZLE_GROUP_ID_TAG << "=\""
                       << filament_nozzle_group_id << "\" " << FILAMENT_NOZZLE_DIAMETER_TAG << "=\"" << filament_nozzle_diameter << "\" "
                       << FILAMENT_NOZZLE_VOLUME_TYPE_TAG << "=\"" << filament_nozzle_volume_type << "\" " << FILAMENT_USED_FOR_OBJECT
                       << "=\"" << std::boolalpha << it->used_for_object << "\" " << FILAMENT_USED_FOR_SUPPORT << "=\"" << std::boolalpha
                       << it->used_for_support << "\"/>\n";
            }
            ...
            if (plate_data->nozzle_group_result) {
                ...
                for (auto& used_nozzle : used_nozzle_list) {
                    if (!has_multi_nozzle_extruder && nozzle_diameter_option && used_nozzle.extruder_id >= 0 &&
                        used_nozzle.extruder_id < (int) nozzle_diameter_option->values.size()) {
            ...
            if (!plate_data->layer_filaments.empty()) {
                ...
                for (auto iter = plate_data->layer_filaments.begin(); iter != plate_data->layer_filaments.end(); ++iter) {
                    ...
                    stream << "      <" << LAYER_FILAMENT_LIST_TAG << " filament_list=\"" << key_stream.str() << "\" layer_ranges=\""
                           << value_stream.str() << "\" />\n";
                }
                stream << "    </" << LAYER_FILAMENT_LISTS_TAG << ">\n";
            }
                  ...
                  int plate_id)
{
    ...
    bool res = importer.load_model_from_file(path, *model, *plate_data_list, *project_presets, *config, *config_substitutions, strategy,
                                             is_bbl_3mf, is_orca_3mf, *file_version, proFn, project, plate_id);
    importer.log_errors();
...
bool to_xml(
    std::stringstream& stream, const EmbossShape::SvgFile& svg, const ModelVolume& volume, mz_zip_archive& archive, bool export_full_path)
{
