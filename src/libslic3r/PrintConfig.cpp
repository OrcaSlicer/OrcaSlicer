...
void PrintConfigDef::init_fff_params()
{
    ...
    */
    ...
    def->tooltip = L("Which pattern the outer (reinforcement) zone uses.\n\n"
                     ...
                     "inscribed circle is usable tube — so rounder cells give more tube per nozzle "
                     "and seal at a shallower depth. Bore as a fraction of that circle the nozzle "
                     "must cover: Triangle 50%, Rectilinear 71%, Hex/Tri-hex 87%.\n\n"
                     ...
                     "height of the tube — a leak path the seal geometry above cannot see. Hex "
                     "sealed poorly on the one plate that tested it, which fits that explanation "
                     "but does not confirm it.\n\n"
                     "Rectilinear is the default: its grid traces as straight lines, so each wall "
                     "is a single bead with no lengthwise join, and it has the best combination of "
                     "print speed and printability. It is also the pattern all the tuning guidance "
                     "was measured on. Triangle seals deepest for a given tube and has the "
                     "narrowest bore.");
    def->enum_keys_map = &ConfigOptionEnum<InfillPattern>::get_enum_values();
    ...
    def->category = L("Strength");
    def->tooltip = L("Width of the outer (reinforcement) infill zone in mm. Larger values create a "
                     "thicker reinforced region around the perimeter.");
    def->sidetext = L("mm");
    def->min = 0;
    def->max = 50;
    def->mode = comAdvanced;
    ...
    // 3 turns rather than 2: the extra pass is cheap and plows the rim back more completely.
    ...
    def->mode = comAdvanced;
    def->set_default_value(new ConfigOptionEnum<MagmaInjectionOrdering>(MagmaInjectionOrdering::SpreadHeat));
