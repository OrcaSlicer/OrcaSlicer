...
std::string GCodeWriter::set_fan(const GCodeFlavor gcode_flavor, unsigned int speed, unsigned int pct_fan_min_pwm)
{
    std::ostringstream gcode;
    ...
    if (speed > 0 && pct_fan_min_pwm > 0 && speed < pct_fan_min_pwm)
        speed = pct_fan_min_pwm;
    if (speed == 0) {
        switch (gcode_flavor) {
        case gcfTeacup:
            gcode << "M106 S0"; break;
...
std::string GCodeWriter::set_fan(unsigned int speed) const
{
    ...
    return GCodeWriter::set_fan(this->config.gcode_flavor, speed,
                                static_cast<unsigned int>(std::max(0, this->config.pct_fan_min_pwm.value)));
}
