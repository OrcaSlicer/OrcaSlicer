...
static void change_memoryed_heaps_to_arrays(FilamentGroupUtils::MemoryedGroupHeap& heap,
...
static double calc_change_time_for_group(const std::vector<int>& filament_change_seq,
...
{
    auto r = MultiNozzleUtils::simulate_filament_change_time(logical_filaments, nozzle_list, filament_change_seq, nozzle_change_seq,
                                                             group_of_filament, time_params, ams_preload_enabled);
    return r.actual_time;
}

static double full_evaluate(const std::vector<unsigned int>& used_filaments,
                            ...
                            int* out_flush                                                            = nullptr)
{
    ...
    int flush = reorder_filaments_for_multi_nozzle_extruder(used_filaments, *group_res, ctx.model_info.layer_filaments,
                                                            ctx.model_info.flush_matrix,
                                                            get_custom_seq ? *get_custom_seq : std::function<bool(int, std::vector<int>&)>{},
                                                            &filament_sequences, initial_status);
    ...
    if (!filament_sequences.empty()) {
        ...
        for (const auto& layer_seq : filament_sequences) {
            for (unsigned int fil : layer_seq) {
                ...
                if ((int) fil == prev_fil && nid == prev_nozzle)
                    continue;
        ...
        change_time = calc_change_time_for_group(filament_change_seq, nozzle_change_seq, logical_filaments, ctx.nozzle_info.nozzle_list,
                                                 ctx.speed_info.change_time_params, ctx.speed_info.ams_preload_enabled, group_of_filament);
    }
...
std::vector<int> FilamentGroup::calc_group_by_enum(int k,
                                                   const std::vector<unsigned int>& used_filaments,
                                                   ...
                                                   int* cost)
{
    ...
    for (long long mask = 0; mask < total; mask++) {
        ...
        for (auto& nf : nozzles_filaments) {
            ...
            for (int filament : nf.second)
                filament_mask |= (1ULL << filament);
        ...
        if (group_set.find(group_hashs) != group_set.end())
            continue;
...
std::vector<int> FilamentGroup::calc_group_by_kmedoids(int k,
                                                       const std::vector<unsigned int>& used_filaments,
                                                       const std::unordered_map<int, std::vector<int>>& unplaceable_limits,
                                                       int* cost)
{
    auto distance_evaluator = std::make_shared<FlushDistanceEvaluator>(ctx.model_info.flush_matrix, used_filaments,
...
std::vector<int> select_best_group_for_ams(const std::vector<std::vector<int>>& filament_to_nozzles,
                                           ...
                                           const std::vector<std::vector<FilamentGroupUtils::MachineFilamentInfo>>& machine_filament_info_,
...
FlushDistanceEvaluator::FlushDistanceEvaluator(const std::vector<FlushMatrix>& flush_matrix,
                                               const std::vector<unsigned int>& used_filaments,
                                               ...
                                               double p)
{
    ...
    m_distance_matrix.resize(flush_matrix.size(),
                             std::vector<std::vector<float>>(used_filaments.size(), std::vector<float>(used_filaments.size())));
        ...
        for (size_t j = 0; j < used_filaments.size(); ++j) {
            for (size_t k = 0; k < flush_matrix.size(); k++) {
                if (i == j)
                    ...
                    float max_val              = std::max(flush_matrix[k][used_filaments[i]][used_filaments[j]],
                                                          flush_matrix[k][used_filaments[j]][used_filaments[i]]);
                    float min_val              = std::min(flush_matrix[k][used_filaments[i]][used_filaments[j]],
                                                          flush_matrix[k][used_filaments[j]][used_filaments[i]]);
...
void KMediods::set_cluster_group_size(const std::vector<std::pair<std::set<int>, int>>& cluster_group_size)
{
    ...
    for (int i = 0; i < m_cluster_group_size.size(); i++) {
        for (auto nozzle_id : m_cluster_group_size[i].first)
            m_nozzle_to_extruder[nozzle_id] = i;
    }
}
...
bool KMediods::have_enough_size(const std::vector<int>& cluster_size,
                                const std::vector<std::pair<std::set<int>, int>>& cluster_group_size,
                                int elem_count)
{
    ...
    if (!cluster_group_size.empty())
        cluster_group_sum = std::accumulate(cluster_group_size.begin(), cluster_group_size.end(), 0,
                                            [](int a, const std::pair<std::set<int>, int>& p) { return a + p.second; });
...
std::vector<int> KMediods::init_cluster_center(const std::unordered_map<int, std::vector<int>>& placeable_limits,
                                               const std::unordered_map<int, std::vector<int>>& unplaceable_limits,
                                               const std::vector<int>& cluster_size,
                                               const std::vector<std::pair<std::set<int>, int>>& cluster_group_size,
                                               int seed)
{
    // max flow network
...
std::vector<int> KMediods::assign_cluster_label(const std::vector<int>& center,
                                                const std::unordered_map<int, std::vector<int>>& placeable_limits,
                                                const std::unordered_map<int, std::vector<int>>& unplaceable_limits,
                                                const std::vector<int>& cluster_size,
                                                const std::vector<std::pair<std::set<int>, int>>& cluster_group_size)
{
    ...
    MinFlushFlowSolver M(distance_matrix, l_nodes, r_nodes, placeable_limits, unplaceable_limits, l_nodes_capacity, r_nodes_capacity,
                         r_nodes_group_capacity);
    auto ret = M.solve();
...
void KMediods::do_clustering(const FilamentGroupContext& context, const ClusteringBudget& budget)
{
    ...
    while (retry_count < retry && within_budget()) {
        std::vector<int> curr_cluster_centers = init_cluster_center(m_placeable_limits, m_unplaceable_limits, m_max_cluster_size,
                                                                    m_cluster_group_size, retry_count);
        std::vector<int> curr_cluster_labels  = assign_cluster_label(curr_cluster_centers, m_placeable_limits, m_unplaceable_limits,
                                                                     m_max_cluster_size, m_cluster_group_size);
        ...
        while (mediods_changed && within_budget()) {
            ...
            for (size_t cluster_id = 0; cluster_id < m_k; ++cluster_id) {
                if (curr_cluster_centers[cluster_id] == -1)
                    continue;
                for (int elem = 0; elem < m_elem_count; ++elem) {
                    if (std::find(curr_cluster_centers.begin(), curr_cluster_centers.end(), elem) != curr_cluster_centers.end() ||
                        std::find(m_unplaceable_limits[cluster_id].begin(), m_unplaceable_limits[cluster_id].end(), elem) !=
                            m_unplaceable_limits[cluster_id].end())
                        continue;
                    std::vector<int> tmp_centers = curr_cluster_centers;
                    tmp_centers[cluster_id]      = elem;
                    std::vector<int> tmp_labels  = assign_cluster_label(tmp_centers, m_placeable_limits, m_unplaceable_limits,
                                                                        m_max_cluster_size, m_cluster_group_size);
            ...
            if (mediods_changed) {
                ...
                curr_cluster_labels = assign_cluster_label(curr_cluster_centers, m_placeable_limits, m_unplaceable_limits,
                                                           m_max_cluster_size, m_cluster_group_size);
...
std::map<int, int> FilamentGroup::rebuild_unprintables(const std::vector<unsigned int>& used_filaments,
                                                       const std::map<int, int>& extruder_unprintables)
{
    std::map<int, int> ret;
...
std::vector<int> FilamentGroup::calc_filament_group_for_match(int* cost)
{
    ...
    auto optimize_map_to_machine_filament = [&](const std::vector<int>& map_to_machine_filament, const std::vector<int>& l_nodes,
                                                const std::vector<int>& r_nodes, std::vector<int>& filament_map) {
...
std::vector<int> FilamentGroup::calc_filament_group_for_flush(int* cost)
{
    ...
    ret = select_best_group_for_ams(memoryed_maps, ctx.nozzle_info.nozzle_list, used_filaments, used_filament_info,
                                    ctx.machine_info.machine_filament_info, ctx.group_info.has_filament_switcher);
    return ret;
}
...
std::vector<int> FilamentGroup::calc_filament_group_for_tpu(int* cost)
{
    ...
    std::vector<std::vector<float>> print_time_matrix(used_filaments.size(),
                                                      std::vector<float>(ctx.nozzle_info.extruder_nozzle_list.size()));
    for (int i = 0; i < used_filaments.size(); ++i) {
        for (int j = 0; j < ctx.nozzle_info.extruder_nozzle_list.size(); ++j) {
            ...
            if (ctx.nozzle_info.nozzle_list[j].volume_type ==
                nvtTPUHighFlow) // when both TPU High Flow and other nozzle types exist, prefer assigning filament to the TPU High Flow nozzle
    ...
    for (int i = 0; i < used_filaments.size(); i++) {
        ...
        if (iter == unprintable_limit_indices.end() || iter->second < 0 || iter->second >= 2)
            continue;
    ...
    for (int i = 0; i < ret.size(); ++i)
        group[used_filaments[i]] = ret[i];
    return group;
}

// sorted used_filaments

std::unordered_map<int, std::vector<int>> FilamentGroup::rebuild_nozzle_unprintables(
    const std::vector<unsigned int>& used_filaments,
    const std::unordered_map<int, std::vector<int>>& extruder_unprintables,
    const std::vector<int>& filament_volume_map)
{
    ...
    for (size_t fidx = 0; fidx < used_filaments.size(); ++fidx) {
        ...
        for (size_t nozzle_idx = 0; nozzle_idx < ctx.nozzle_info.nozzle_list.size(); ++nozzle_idx) {
            ...
            if (std::find(unexpected_extruders.begin(), unexpected_extruders.end(), nozzle_info.extruder_id) != unexpected_extruders.end() ||
                (expected_volume != nvtHybrid && expected_volume != nozzle_info.volume_type) ||
...
std::vector<FilamentPlanRes> plan_filament_nozzle_mapping_and_order(const FilamentGroupContext& ctx)
{
    ...
    for (int r_id = 0; r_id < r_nodes.size(); r_id++) {
        if (initial_nozzle.count(r_id))
            r_nodes[r_id] = initial_nozzle[r_id];
    }

    std::vector<int> r_nodes_group(ctx.nozzle_info.nozzle_list.size(), -1);
    for (auto& nozzle_info : ctx.nozzle_info.nozzle_list) {
        r_nodes_group[nozzle_info.group_id] = nozzle_info.extruder_id;
    }
    ...
    for (int i = 0; i < layer_nums; i++) {
        ...
        for (size_t noz_id = 0; noz_id < r_nodes.size(); ++noz_id) {
            if (r_nodes[noz_id] >= 0) {
                filament_to_nozzle[r_nodes[noz_id]] = noz_id;
            }
        }
        ...
        for (int m = 0; m < ctx.nozzle_info.extruder_nozzle_list.size(); m++) {
            ...
            for (auto noz_id : ctx.nozzle_info.extruder_nozzle_list.at(cur_extruder)) {
                if (nozzle_fil_deq[noz_id].empty())
                    continue;
