...
ConfigSubstitutions ConfigBase::load_from_gcode_file(const std::string &file, ForwardCompatibilitySubstitutionRule compatibility_rule)
{
    ...
    bool has_delimiters = true;
    {
        ...
        // Accept "Orca_Slicer" as an alias of the application name: G-code that carries
        // that producer tag must still load its config block (Creality K2 SE workflow).
        std::string orcaslicer_alias_gcode_header = "; generated by Orca_Slicer";
        ...
        while (std::getline(ifs, header)) {
            ...
            if (strncmp(bambuslicer_gcode_header.c_str(), line_c, strlen(bambuslicer_gcode_header.c_str())) == 0 ||
                ...
                strncmp(orcaslicer_alias_gcode_header.c_str(), line_c, strlen(orcaslicer_alias_gcode_header.c_str())) == 0) {
                header_found = true;
                break;
            }
        }
