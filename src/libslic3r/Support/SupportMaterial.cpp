...
Point export_support_surface_type_legend_to_svg_box_size() { return Point(scale_(1. + 10. * 8.), scale_(3.)); }
...
void export_print_z_polygons_and_extrusions_to_svg(const char* path,
...
#ifdef SUPPORT_USE_AGG_RASTERIZER
static std::vector<unsigned char> rasterize_polygons(const Vec2i32& grid_size,
...
static Polygons contours_simplified(const Vec2i32& grid_size,
                                    const double pixel_size,
                                    Point left_bottom,
    ...
    if (fill_holes) {
        ...
        for (int r = 1; r + 1 < grid_size.y(); ++r) {
            for (int c = 1; c + 1 < grid_size.x(); ++c) {
                ...
                if ((grid[addr - 1] != 0 && grid[addr + 1] != 0) || (grid[addr - grid_size.x()] != 0 && grid[addr + grid_size.x()] != 0))
...
inline SupportGeneratorLayer& layer_allocate(std::deque<SupportGeneratorLayer>& layer_storage, SupporLayerType layer_type)
{
...
inline SupportGeneratorLayer& layer_allocate(std::deque<SupportGeneratorLayer>& layer_storage,
...
void PrintObjectSupportMaterial::generate(PrintObject& object)
{
    ...
    for (const SupportGeneratorLayer* layer : top_contacts)
        Slic3r::SVG::export_expolygons(debug_out_path("support-top-contacts-%d-%lf.svg", iRun, layer->print_z), union_ex(layer->polygons));
#endif /* SLIC3R_DEBUG */
    ...
    SupportGeneratorLayersPtr bottom_contacts = this->bottom_contact_layers_and_layer_support_areas(object, top_contacts,
                                                                                                    buildplate_covered, layer_storage,
                                                                                                    layer_support_areas);
    ...
    for (size_t layer_id = 0; layer_id < object.layers().size(); ++layer_id)
        Slic3r::SVG::export_expolygons(debug_out_path("support-areas-%d-%lf.svg", iRun, object.layers()[layer_id]->print_z),
    ...
    SupportGeneratorLayersPtr intermediate_layers = this->raft_and_intermediate_support_layers(object, bottom_contacts, top_contacts,
                                                                                               layer_storage);

    this->trim_support_layers_by_object(object, top_contacts, m_slicing_params.gap_support_object, m_slicing_params.gap_object_support,
                                        m_support_params.gap_xy);

#ifdef SLIC3R_DEBUG
    for (const SupportGeneratorLayer* layer : top_contacts)
        Slic3r::SVG::export_expolygons(debug_out_path("support-top-contacts-trimmed-by-object-%d-%lf.svg", iRun, layer->print_z),
    ...
    for (SupportGeneratorLayersPtr::const_iterator it = intermediate_layers.begin(); it != intermediate_layers.end(); ++it)
        Slic3r::SVG::export_expolygons(debug_out_path("support-base-layers-%d-%lf.svg", iRun, (*it)->print_z), union_ex((*it)->polygons));
#endif /* SLIC3R_DEBUG */
    ...
    auto [interface_layers, base_interface_layers] = generate_interface_layers(*m_object_config, m_support_params, bottom_contacts,
                                                                               top_contacts, empty_layers, empty_layers,
    ...
    SupportGeneratorLayersPtr raft_layers = generate_raft_base(object, m_support_params, m_slicing_params, top_contacts, interface_layers,
                                                               base_interface_layers, intermediate_layers, layer_storage);
    ...
    for (const SupportGeneratorLayer* l : interface_layers)
        Slic3r::SVG::export_expolygons(debug_out_path("support-interface-layers-%d-%lf.svg", iRun, l->print_z), union_ex(l->polygons));
    for (const SupportGeneratorLayer* l : base_interface_layers)
        Slic3r::SVG::export_expolygons(debug_out_path("support-base-interface-layers-%d-%lf.svg", iRun, l->print_z), union_ex(l->polygons));
#endif // SLIC3R_DEBUG
    ...
    */
...
#endif // SLIC3R_DEBUG
        generate_support_layers(object, raft_layers, bottom_contacts, top_contacts, intermediate_layers, interface_layers,
    ...
    generate_support_toolpaths(object.support_layers(), *m_object_config, m_support_params, m_slicing_params, raft_layers, bottom_contacts,
                               top_contacts, intermediate_layers, interface_layers, base_interface_layers);
        ...
        for (int i = 0; i < int(layers_sorted.size());) {
            ...
            for (; j < layers_sorted.size() && layers_sorted[j]->print_z <= zmax; ++j)
                if (!layers_sorted[j]->polygons.empty())
                    empty = false;
            if (!empty) {
                export_print_z_polygons_to_svg(debug_out_path("support-%d-%lf.svg", iRun, layers_sorted[i]->print_z).c_str(),
                                               layers_sorted.data() + i, j - i);
                export_print_z_polygons_and_extrusions_to_svg(debug_out_path("support-w-fills-%d-%lf.svg", iRun, layers_sorted[i]->print_z)
                                                                  .c_str(),
...
public:
        ...
        , m_support_material_closing_radius(params.support_closing_radius)
    {
        ...
        case smsGrid: {
            ...
            seed_fill_block(m_grid2, m_grid_size,
                            dilate_trimming_region(rasterize_polygons(m_grid_size, m_pixel_size, m_bbox.min, *m_trimming_polygons),
                                                   m_grid_size),
            ...
            {
                static int irun;
                Slic3r::png::write_gray_to_file_scaled(debug_out_path("support-rasterizer-%d.png", irun++), m_grid_size.x(),
                                                       m_grid_size.y(), m_grid2.data(), 4);
            }
#endif // SLIC3R_DEBUG
    ...
    )
    {
        ...
        case smsGrid: {
#ifdef SUPPORT_USE_AGG_RASTERIZER
            Polygons support_polygons_simplified = contours_simplified(m_grid_size, m_pixel_size, m_bbox.min, m_grid2, offset_in_grid,
                                                                       fill_holes);
#else  // SUPPORT_USE_AGG_RASTERIZER
       // Generate islands, so each island may be tested for overlap with island_samples.
            ...
            Points samples = island_samples(offset_in_grid > 0 ?
                ...
                if (!samples_inside.empty()) {
                    ...
                    for (size_t i_contour = 0; i_contour <= island.holes.size(); ++i_contour) {
                        ...
                        for (; i != contour.points.end(); j = i++) {
                            // FIXME this test is not numerically robust. Particularly, it does not handle horizontal segments at y ==
                            // point(1) well.
...
#endif /* SLIC3R_DEBUG */
            // FIXME do we want to trim with the object here? On one side the columns will be thinner, on the other side support interfaces
            // may disappear for snug supports.
            //  return diff(smooth_outward(closing(*m_support_polygons, closing_distance, SUPPORT_SURFACES_OFFSET_PARAMETERS),
            //  smoothing_distance), *m_trimming_polygons);
...
private:
    ...
    static std::vector<unsigned char> dilate_trimming_region(const std::vector<unsigned char>& trimming, const Vec2i32& grid_size)
    {
        ...
        for (int r = 1; r + 1 < grid_size.y(); ++r)
            for (int c = 1; c + 1 < grid_size.x(); ++c) {
                ...
                //  dilated[addr] = trimming[addr] != 0 && trimming[addr - 1] != 0 && trimming[addr + 1] != 0 && trimming[addr -
                //  m_grid_size.x()] != 0 && trimming[addr + m_grid_size.x()] != 0; 8-neighborhood
    ...
    static void seed_fill_block(std::vector<unsigned char>& grid,
                                Vec2i32 grid_size,
                                const std::vector<unsigned char>& trimming,
                                const Vec2i32& grid_blocks,
                                int oversampling)
    {
...
static void remove_bridges_from_contacts(const PrintConfig& print_config,
                                         ...
                                         Polygons& contact_polygons)
{
    ...
    Polygons bridges;
    {
        ...
        // FIXME one may want to use a maximum of bridging flow width and normal flow width, as the perimeters are calculated using the
        // normal flow
        //  and then turned to bridging flow, thus their centerlines are derived from non-bridging flow and expanding them by a bridging
        //  flow may not expand them to the edge of their respective islands.
        const float w = float(0.5 * std::max(perimeter_bridge_flow.scaled_width(), perimeter_bridge_flow.scaled_spacing())) +
                        scaled<float>(0.001);
        for (Polyline& polyline : overhang_perimeters)
...
static inline ExPolygons detect_overhangs(const Layer& layer,
...
)
{
    ...
    if (layer_id == 0) {
        ...
        for (LayerRegion* layerm : layer.regions()) {
            ...
            lower_layer_offset = (layer_id < (size_t) object_config.enforce_support_layers.value) ?
            ...
            if (lower_layer_offset == 0.f) {
                ...
                diff_polygons = diff(layerm_polygons, expand(lower_layer_polygons, lower_layer_offset, SUPPORT_SURFACES_OFFSET_PARAMETERS));
                ...
                if (!diff_polygons.empty()) {
                    ...
                    diff_polygons = diff(intersection(expand(diff_polygons, lower_layer_offset, SUPPORT_SURFACES_OFFSET_PARAMETERS),
                                                      layerm_polygons),
                ...
                if (g_config_support_sharp_tails) {
                    for (ExPolygon& expoly : layerm->raw_slices) {
                        if (offset_ex(expoly, -0.5 * fw).empty())
                            continue;
            ...
            if (bridge_no_support) {
                ...
                SupportMaterialInternal::remove_bridges_from_contacts(print_config, lower_layer, lower_layer_polygons, *layerm, fw,
                                                                      diff_polygons);
            }
    ...
    if (layer.lower_layer) {
        for (ExPolygon& poly : overhang_areas) {
            float fw                  = float(layer.regions().front()->flow(frExternalPerimeter).scaled_width());
            auto cluster_boundary_ex  = intersection_ex(poly, offset_ex(layer.lower_layer->lslices,
                                                                        std::max(fw, lower_layer_offset) + scale_(0.1)));
...
// Tuple: overhang_polygons, contact_polygons, enforcer_polygons, no_interface_offset
// no_interface_offset: minimum of external perimeter widths
static inline std::tuple<Polygons, Polygons, double> detect_contacts(const Layer& layer,
                                                                     const size_t layer_id,
                                                                     Polygons& overhang_polygons,
                                                                     Polygons& lower_layer_polygons,
                                                                     const PrintConfig& print_config,
...
)
{
    ...
    if (layer_id == 0) {
        ...
        auto slices_margin_update = [&slices_margin, &layer, &lower_layer, &lower_layer_polygons, buildplate_only, has_enforcer,
            ...
            if (slices_margin.offset != slices_margin_offset) {
                ...
                slices_margin.polygons = (slices_margin_offset == 0.f) ?
                                             lower_layer_polygons :
                                             // What is the purpose of no_interface_offset? Likely to not trim the contact layer by lower
                                             // layer regions that are too thin to extrude?
        ...
        no_interface_offset = std::accumulate(layer.regions().begin(), layer.regions().end(), FLT_MAX,
                                              [](float acc, const LayerRegion* layerm) {
            ...
            // Enforce supports (as if with 90 degrees of slope) for the regions covered by the enforcer meshes.
                ...
                SVG::export_expolygons(debug_out_path("support-top-contacts-enforcers-run%d-layer%d-z%f.svg", iRun, layer_id, layer.print_z),
                                       {{layer.lslices, {"layer.lslices", "gray", 0.2f}},
                                        ...
                                        {{union_safety_offset_ex(enforcer_polygons)},
                                         {"new_contacts", "red", "black", "", scaled<coord_t>(0.1f), 0.5f}}});
#endif /* SLIC3R_DEBUG */
...
static inline std::pair<SupportGeneratorLayer*, SupportGeneratorLayer*> new_contact_layer(const PrintConfig& print_config,
                                                                                          const PrintObjectConfig& object_config,
            ...
            if (bridging_print_z >= min_print_z) {
                ...
                if (bridging_print_z < print_z - EPSILON) {
                    ...
                    if (bridging_print_z == slicing_params.first_print_layer_height) {
                        ...
                        // BBS: if independent_support_layer_height is not enabled, the support layer_height should be the same as layer
                        // height. Note that for this case, adaptive layer height must be disabled.
...
static inline void fill_contact_layer(SupportGeneratorLayer& new_layer,
                                      size_t layer_id,
                                      const SlicingParameters& slicing_params,
                                      const PrintObjectConfig& object_config,
...
)
{
    ...
    auto lower_layer_polygons_for_dense_interface = [&lower_layer_polygons_for_dense_interface_cache, &lower_layer_polygons,
                                                     no_interface_offset]() -> const Polygons& {
        if (lower_layer_polygons_for_dense_interface_cache.empty())
            lower_layer_polygons_for_dense_interface_cache =
                // FIXME no_interface_offset * 0.6f offset is not quite correct, one shall derive it based on an angle thus depending on
                // layer height.
    ...
    // 1) Contact polygons will be projected down. To keep the interface and base layers from growing, return a contour a tiny bit smaller
    // than the grid cells.
...
#ifdef SLIC3R_DEBUG
                                                                                                 ,
                                                                                                 "top_contact_polygons", iRun, layer_id,
                                                                                                 layer.print_z
#endif // SLIC3R_DEBUG
                                                                                                 ));
    ...
    if (!enforcer_polygons.empty() && !slices_margin.all_polygons.empty() && layer_id > 0) {
        ...
        // 1) Contact polygons will be projected down. To keep the interface and base layers from growing, return a contour a tiny bit
        // smaller than the grid cells.
        new_layer.enforcer_polygons = std::make_unique<Polygons>(
        ...
        if (reduce_interfaces) {
            ...
            if (!dense_interface_polygons.empty()) {
                dense_interface_polygons = diff(
                    // Regularize the contour.
                    expand(dense_interface_polygons, no_interface_offset * 0.1f), slices_margin.all_polygons);
                // Support islands, to be stretched into a grid.
                // FIXME The regularization of dense_interface_polygons above may stretch dense_interface_polygons outside of the contact
                // polygons,
                // thus some dense interface areas may not get supported. Trim the excess with contact_polygons at the following line.
                // See for example GH #4874.
...
// Merge close contact layers conservatively: If two layers are closer than the minimum allowed print layer height (the min_layer_height
// parameter), the top contact layer is merged into the bottom contact layer.
static void merge_contact_layers(const SlicingParameters& slicing_params, double support_layer_height_min, SupportGeneratorLayersPtr& layers)
{
    ...
    std::sort(layers.begin(), layers.end(),
              [](const SupportGeneratorLayer* l1, const SupportGeneratorLayer* l2) { return l1->print_z < l2->print_z; });
    ...
    int k = 0;
    {
        ...
        int j = 0;
        for (; j < (int) layers.size() && layers[j]->print_z < slicing_params.first_print_layer_height + support_layer_height_min - EPSILON;
             ++j)
            ;
        if (j > 0) {
...
#define SLIC3R_IRUN , iRun
    ...
    if (m_object_config->enable_support.value &&
        (m_object_config->support_type.value != stNormalAuto && m_object_config->support_type.value != stNormal)) {
    ...
    tbb::parallel_for(tbb::blocked_range<size_t>(layer_id_start, num_layers), [&](const tbb::blocked_range<size_t>& range) {
        for (size_t layer_id = range.begin(); layer_id < range.end(); layer_id++) {
            const Layer& layer            = *object.layers()[layer_id];
            Polygons lower_layer_polygons = (layer_id == 0) ? Polygons() : to_polygons(object.layers()[layer_id - 1]->lslices);

            overhangs_per_layers[layer_id] = detect_overhangs(layer, layer_id, lower_layer_polygons, *m_print_config, *m_object_config,
                                                              annotations, m_support_params.gap_xy
#ifdef SLIC3R_DEBUG
                                                              ,
                                                              iRun
#endif // SLIC3R_DEBUG
    ...
    const coordf_t extrusion_width    = m_object_config->line_width.get_abs_value(
        object.print()->config().nozzle_diameter.get_at(object.config().support_interface_filament - 1));
        ...
        for (size_t layer_nr = layer_id_start; layer_nr < num_layers; layer_nr++) {
            ...
            for (const ExPolygon& expoly : layer->lslices) {
                ...
                do {
                    ...
                    if (size_diff.both_comp(Point(scale_(5), scale_(5)), ">") ||
    ...
    if (config_remove_small_overhangs) {
        ...
        for (OverhangCluster& cluster : clusters) {
            ...
            SVG svg(format("SVG/overhangCluster_%s_%s_tail=%s_cantilever=%s_small=%s.svg", cluster.min_layer, layer1->print_z,
    ...
    for (size_t layer_id = layer_id_start; layer_id < num_layers; layer_id++) {
        ...
        auto [contact_polygons, enforcer_polygons,
              no_interface_offset] = detect_contacts(layer, layer_id, overhang_polygons, lower_layer_polygons, *m_print_config,
                                                     *m_object_config, annotations, slices_margin, m_support_params.gap_xy
#ifdef SLIC3R_DEBUG
                                                     ,
                                                     iRun
#endif // SLIC3R_DEBUG
        ...
        if (!contact_polygons.empty() || !overhang_polygons.empty()) {
            ...
            auto [new_layer, bridging_layer] = new_contact_layer(*m_print_config, *m_object_config, m_slicing_params,
                                                                 m_support_params.support_layer_height_min, layer, layer_storage);
            if (new_layer) {
                // Fill the non-bridging layer with polygons.
                fill_contact_layer(*new_layer, layer_id, m_slicing_params, *m_object_config, slices_margin, overhang_polygons,
                                   contact_polygons, enforcer_polygons, lower_layer_polygons, m_support_params.support_material_flow,
                                   no_interface_offset
#ifdef SLIC3R_DEBUG
                                   ,
                                   iRun, layer
                ...
                // Insert new layer even if there is no interface generated: Likely the support angle is not steep enough to require dense
                // interface, however generating a sparse support will be useful for the object stability. if (!
                // new_layer->polygons.empty())
        ...
        for (size_t top_idx = size_t(std::max<int>(0, contact_idx));
            ...
            if (top_contacts[top_idx]->print_z > layer_new.print_z - support_params.support_layer_height_min - EPSILON) {
                ...
                if (diff > 0.F) {
                    ...
                    // By making the layer thicker, one will decrease the number of discrete layers with the price of extruding a bit too
                    // thick bridges. By making the layer thinner, one adds one more discrete layer.
                    layer_new.print_z = top_contacts[top_idx]->print_z;
                    layer_new.height -= diff;
                }
                break;
...
#ifdef SLIC3R_DEBUG
            SVG::export_expolygons(debug_out_path("support-support-areas-raw-before-trimming-%d-with-%f-%lf.svg", iRun, layer.print_z,
                                                  layer_above.print_z),
...
#ifdef SLIC3R_DEBUG
            Slic3r::SVG::export_expolygons(debug_out_path("support-support-areas-raw-after-trimming-%d-with-%f-%lf.svg", iRun,
...
// Called twice: First for normal supports, possibly trimmed by "on build plate only", second for support enforcers not trimmed by "on build
// plate only".
...
#ifdef SLIC3R_DEBUG
        Slic3r::SVG::export_expolygons(debug_out_path("support-layer_support_area-gridded-%s-%d-%lf.svg", debug_name, iRun, layer.print_z),
                                       union_ex(out.first));
#endif /* SLIC3R_DEBUG */
    });

    // 2) Support polygons will be projected down. To keep the interface and base layers from growing, return a contour a tiny bit smaller
    // than the grid cells.
    task_group_inner.run([&grid_params, &support_grid_pattern, &out
#ifdef SLIC3R_DEBUG
                          ,
...
#ifdef SLIC3R_DEBUG
        Slic3r::SVG::export_expolygons(debug_out_path("support-projection_new-gridded-%d-%lf.svg", iRun, layer.print_z),
                                       union_ex(out.second));
#endif /* SLIC3R_DEBUG */
#ifdef SLIC3R_DEBUG
    ...
    std::vector<Polygons>& layer_support_areas) const
{
    ...
    for (int layer_id = int(object.total_layer_count()) - 2; layer_id >= 0; --layer_id) {
       ...
       // Use a slight positive offset to overlap the touching regions.
        ...
        if (!overhangs_for_bottom_contacts.empty())
            // Find the bottom contact layers above the top surfaces of this layer.
            task_group.run([this, &object, &layer, &top_contacts, contact_idx, &layer_storage, &layer_support_areas, &bottom_contacts,
                ...
                // Find the bottom contact layers above the top surfaces of this layer.
                SupportGeneratorLayer* layer_new = detect_bottom_contacts(m_slicing_params, m_support_params, object, layer, top_contacts,
                                                                          contact_idx, layer_storage, layer_support_areas,
                                                                          overhangs_for_bottom_contacts
#ifdef SLIC3R_DEBUG
                                                                          ,
                                                                          iRun, polygons_new
        ...
        //        float column_propagation_filtering_radius = scaled<float>(0.8 * 0.5 * (m_support_params.support_material_flow.spacing() +
        //        m_support_params.support_material_flow.width()));
        task_group.run([&grid_params, &overhangs_projection, &overhangs_projection_raw, &layer, &layer_support_area,
            ...
            std::tie(layer_support_area, overhangs_projection) = project_support_to_grid(layer, grid_params, overhangs_projection_raw,
        ...
        if (!enforcers_projection.empty())
            ...
            task_group.run([&grid_params, &enforcers_projection, &enforcers_projection_raw, &layer, &layer_support_area_enforcers
                ...
                std::tie(layer_support_area_enforcers, enforcers_projection) = project_support_to_grid(layer, grid_params,
    ...
    trim_support_layers_by_object(object, bottom_contacts, m_slicing_params.gap_support_object, m_slicing_params.gap_object_support,
                                  m_support_params.gap_xy);
...
{
    tbb::parallel_for(tbb::blocked_range<int>(0, int(top_contacts.size())), [&bottom_contacts,
                                                                             &top_contacts](const tbb::blocked_range<int>& range) {
        ...
        for (int idx_top = range.end() - 1; idx_top >= range.begin(); --idx_top) {
            ...
            idx_bottom_overlapping_first = idx_lower_or_equal(bottom_contacts, idx_bottom_overlapping_first,
...
SupportGeneratorLayersPtr PrintObjectSupportMaterial::raft_and_intermediate_support_layers(const PrintObject& object,
                                                                                           const SupportGeneratorLayersPtr& bottom_contacts,
                                                                                           const SupportGeneratorLayersPtr& top_contacts,
                                                                                           SupportGeneratorLayerStorage& layer_storage) const
{
    ...
    for (size_t i = 1; i < extremes.size(); ++i) {
        ...
        assert(extremes[i]->extreme_z() - extremes[i - 1]->extreme_z() > 0. || extremes[i]->layer_type == extremes[i - 1]->layer_type ||
    ...
    for (size_t idx_extreme = idx_extreme_first; idx_extreme < extremes.size(); ++idx_extreme) {
        ...
        if (synchronize) {
            ...
            for (; idx_layer_object < object.layers().size() && object.layers()[idx_layer_object]->print_z < extr2z + EPSILON;
                 ++idx_layer_object) {
                SupportGeneratorLayer& layer_new = layer_storage.allocate(SupporLayerType::Intermediate);
                                                      ...
                                                      const std::vector<Polygons>& layer_support_areas) const
{
    ...
    tbb::parallel_for(tbb::blocked_range<size_t>(0, intermediate_layers.size()), [&object, &bottom_contacts, &top_contacts,
                                                                                  &intermediate_layers, &layer_support_areas](
        ...
        for (int idx_intermediate = int(range.end()) - 1; idx_intermediate >= int(range.begin()); --idx_intermediate) {
            BOOST_LOG_TRIVIAL(trace) << "Support generator - generate_base_layers - creating layer " << idx_intermediate << " of "
                                     << intermediate_layers.size();
            ...
            // Use the precomputed layer_support_areas. "idx_object_layer_above": above means above since the last iteration, not above
            // after this call.
            ...
            // 3) base.print_z > top.print_z  && base.bottom_z >= top.bottom_z -> Overlap, which will be solved inside generate_toolpaths()
            // by reducing the base layer height where it overlaps the top layer. No trimming needed here. 4) base.print_z > top.bottom_z &&
            // base.bottom_z < top.bottom_z -> Base overlaps with top.bottom_z. This must not happen. 5) base.print_z <= top.print_z  &&
            // base.bottom_z >= top.bottom_z -> Base is fully inside top. Trim base by top.
            ...
            // 3) base.print_z > bottom.bottom_z && base.bottom_z < bottom.bottom_z -> Overlap, which will be solved inside
            // generate_toolpaths() by reducing the bottom layer height where it overlaps the base layer. No trimming needed here. 4)
            // base.print_z > bottom.print_z  && base.bottom_z >= bottom.print_z -> Base overlaps with bottom.print_z. This must not happen.
            ...
            if (polygons_trimming.empty())
                ...
                layer_intermediate.polygons = diff(polygons_new, polygons_trimming,
...
#ifdef SLIC3R_DEBUG
    for (SupportGeneratorLayersPtr::const_iterator it = intermediate_layers.begin(); it != intermediate_layers.end(); ++it)
        ::Slic3r::SVG::export_expolygons(debug_out_path("support-intermediate-layers-untrimmed-%d-%lf.svg", iRun, (*it)->print_z),
                                                               ...
                                                               const coordf_t gap_xy) const
{
    ...
    tbb::parallel_for(tbb::blocked_range<size_t>(0, nonempty_layers.size()), [this, &object, &nonempty_layers, gap_extra_above,
                                                                              gap_extra_below,
                                                                              gap_xy_scaled](const tbb::blocked_range<size_t>& range) {
        size_t idx_object_layer_overlapping = size_t(-1);

        auto is_layers_overlap = [](const SupportGeneratorLayer& support_layer, const Layer& object_layer,
                                    coordf_t bridging_height = 0.f) -> bool {
        ...
        for (size_t idx_layer = range.begin(); idx_layer < range.end(); ++idx_layer) {
            ...
            // BOOST_LOG_TRIVIAL(trace) << "Support generator - trim_support_layers_by_object - trimmming non-empty layer " << idx_layer <<
            // " of " << nonempty_layers.size();
            ...
            idx_object_layer_overlapping = Layer::idx_higher_or_equal(object.layers().begin(), object.layers().end(),
                                                                      idx_object_layer_overlapping, [z_threshold](const Layer* layer) {
