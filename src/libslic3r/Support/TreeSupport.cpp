...
void TreeSupport::detect_overhangs(bool check_support_necessity/* = false*/)
{
    ...
    const int thresh_layers_below = 10 / config.layer_height; // sus for vhls
    ...
    for (int layer_nr = 0; layer_nr < m_object->layer_count(); layer_nr++) {
        ...
        if (layer_nr < enforcers.size() && lower_layer) {
            ExPolygons enforced_overhangs = intersection_ex(diff_ex(layer->lslices_extrudable, lower_layer->lslices_extrudable),
                                                            enforcers[layer_nr]);
        ...
        for (size_t i = 0; i < layer->loverhangs.size(); i++)
            overhang_types.emplace(&layer->loverhangs[i], i < nDetected ? OverhangType::Detected :
                                                          i < nEnforced ? OverhangType::Enforced :
...
void TreeSupport::create_tree_support_layers()
{
    ...
    if (m_raft_layers > 0) { // create raft layers
        ...
        coordf_t raft_slice_z = 0.f;
        {
            ...
            Layer*   layer  = m_object->get_layer(layer_id);
            raft_print_z += layer->height;
            raft_slice_z = raft_print_z - layer->height / 2;
            m_object->add_tree_support_layer(layer_id++, layer->height, raft_print_z, raft_slice_z);
        }
...
void TreeSupport::generate_toolpaths()
{
    ...
    size_t   layer_nr = 0;
    coordf_t expand_offset;
    Flow     support_flow;
    coordf_t interface_spacing, bottom_interface_spacing, interface_density, bottom_interface_density;

    float old_layer_height = 0.00f;
    for (; layer_nr < m_slicing_params.base_raft_layers; layer_nr++) {
        SupportLayer* ts_layer                          = m_object->get_support_layer(layer_nr);
        auto          m_support_material_interface_flow = support_material_interface_flow(m_object, float(ts_layer->height));
        if (old_layer_height != ts_layer->height || layer_nr == 0) {
            old_layer_height         = ts_layer->height;
            expand_offset            = (layer_nr == 0 ? m_object_config->raft_first_layer_expansion.value : 0.);
            support_flow             = Flow(support_extrusion_width, ts_layer->height, nozzle_diameter);
            interface_spacing        = object_config.support_interface_spacing.value + m_support_material_interface_flow.spacing();
            bottom_interface_spacing = object_config.support_bottom_interface_spacing.value + m_support_material_interface_flow.spacing();
            bottom_interface_density = std::min(1., m_support_material_interface_flow.spacing() / bottom_interface_spacing);
            interface_density        = std::min(1., m_support_material_interface_flow.spacing() / interface_spacing);
        }
    ...
    for (; layer_nr < m_raft_layers; layer_nr++) {
        ...
        for (auto& poly : first_non_raft_base)
            make_perimeter_and_infill(ts_layer->support_fills.entities, poly, std::min(size_t(1), wall_count), support_flow,
                                      erSupportMaterial, filler_raft, interface_density, false);
    }
    ...
    */
    tbb::parallel_for(tbb::blocked_range<size_t>(m_raft_layers, m_object->support_layer_count()), [&](const tbb::blocked_range<size_t>&
                                                                                                          range) {
        for (size_t layer_id = range.begin(); layer_id < range.end(); layer_id++) {
            if (m_object->print()->canceled())
                break;
            auto filler_interface    = std::unique_ptr<Fill>(Fill::new_from_type(m_support_params.contact_fill_pattern));
            auto filler_Roof1stLayer = std::unique_ptr<Fill>(Fill::new_from_type(ipRectilinear));
            ...
            coordf_t real_interface_spacing = object_config.support_interface_spacing.value + interface_flow.spacing();

            coordf_t real_interface_density = std::min(1., interface_flow.spacing() / real_interface_spacing);

            coordf_t real_bottom_spacing = object_config.support_bottom_interface_spacing.value + interface_flow.spacing();
            coordf_t real_bottom_density = std::min(1., interface_flow.spacing() / real_bottom_spacing);
            // usare real_bottom_density per FloorType
            coordf_t real_bottom_interface_density = std::min(1., interface_flow.spacing() / real_bottom_spacing);
            ...
            for (auto& area_group : ts_layer->area_groups) {
                ...
                if (area_group.type != SupportLayer::BaseType) {
                    ...
                    fill_params.density     = real_interface_density;
                    fill_params.dont_adjust = true;
                }
                if (area_group.type == SupportLayer::Roof1stLayer) {
                    ...
                    make_perimeter_and_infill(temp_support_fills->entities, poly, 1, interface_flow, erSupportMaterial,
                                              filler_Roof1stLayer.get(), real_interface_density, false);
                    ...
                    // floor_areas
                    fill_params.density       = real_bottom_interface_density;
                    ...
                    if (layer_id == 0) {
                        ...
                        fill_expolygons_with_sheath_generate_paths(ts_layer->support_fills.entities, loops, filler_support.get(), density,
                                                                   erSupportMaterial, flow, m_support_params, true, false);
                    } else {
            ...
            if (m_support_params.base_fill_pattern == ipLightning) {
                ...
                if (printZ_to_lightninglayer.find(print_z) == printZ_to_lightninglayer.end())
                    continue;
                // TODO:
                // 1.the second parameter of convertToLines seems to decide how long the lightning should be trimmed from its root, so that
                // the root wont overlap/detach the support contour.
                ...
                //  strengthen lightnings while it may make support harder. decide to enable it or not. if yes, proper values for params are
                                               ...
                                               const TreeSupport3D::TreeSupportSettings&    config)
{
    ...
    for (int layer_nr = move_bounds.size() - 1; layer_nr >= 0; layer_nr--) {
        ...
        for (size_t i = 0; i < elements.size(); i++) {
            ...
            // few modification for implementig VLHS
            size_t   obj_layer_nr = state.layer_idx; // o recupera dal move_bounds
            coordf_t real_height  = (obj_layer_nr < print_object.layer_count()) ? print_object.get_layer(obj_layer_nr)->height :
                                                                                  config.layer_height;

            auto node = this->create_node(state.result_on_layer, state.distance_to_top, layer_nr, 0, state.to_buildplate, nullptr,
                                          state.print_z, real_height);
...
coordf_t TreeSupport::calc_branch_radius(coordf_t base_radius, coordf_t mm_to_top, double diameter_angle_scale_factor, bool use_min_distance)
{
    ...
    if (m_object_config->support_interface_top_layers.value > 0 && mm_to_top > base_radius)
...
void TreeSupport::draw_circles() // to implement tree support vhls
{
    const PrintObjectConfig& config   = m_object->config();
    const Print*             print    = m_object->print();
    bool                     has_brim = print->has_brim();
    // int                      bottom_gap_layers = round(m_slicing_params.gap_object_support / m_slicing_params.layer_height); // sus on vhls
    int bottom_gap_layers = 0;
    for (float temp = 0; temp < m_slicing_params.gap_object_support; bottom_gap_layers++)
        temp += m_ts_data->layer_heights[bottom_gap_layers].height;
    ...
    // generate areas
    // const coordf_t layer_height            = config.layer_height.value;                 // useless
    const size_t   top_interface_layers    = config.support_interface_top_layers.value; // idk how to manage to vhls
    const size_t   bottom_interface_layers = config.support_interface_bottom_layers.value < 0 ?
                                                 top_interface_layers :
                                                 config.support_interface_bottom_layers.value; // idk how to manage to vhls
    const double   nozzle_diameter         = m_object->print()->config().nozzle_diameter.get_at(0);
    ...
    tbb::parallel_for(tbb::blocked_range<size_t>(0, m_ts_data->layer_heights.size()), [&](const tbb::blocked_range<size_t>& range) {
        for (size_t layer_nr = range.begin(); layer_nr < range.end(); layer_nr++) {
            if (print->canceled())
                break;
            ...
            for (const SupportNode* p_node : curr_layer_nodes) {
                ...
                if (node.type == ePolygon || (node.distance_to_top < 0 && !node.is_sharp_tail)) {
                    ...
                    if (obj_layer_nr == 0 && m_raft_layers == 0) {
                        double brim_width = !config.tree_support_auto_brim ?
                                                tree_brim_width :
                                                std::max(MIN_BRANCH_RADIUS_FIRST_LAYER,
                                                         std::min(node.radius + node.dist_mm_to_top / (scale * branch_radius) * 0.5,
    ...
    if (with_lightning_infill) {
        ...
        auto     m_support_material_flow = support_material_flow(m_object, m_slicing_params.layer_height); // sus on vhls
...
void TreeSupport::drop_nodes()
{
    ...
    // few- I will move this on the for, for calculate every node that change with layer heght fpr every cicle
    //  The number of layers to be shrinking the circle to create a tip. This produces a 45 degree angle.
    ...
    const size_t   bottom_interface_layers    = config.support_interface_bottom_layers.value < 0 ? top_interface_layers :
    ...
    coordf_t old_layer_height = 0;

    // const coordf_t max_move_distance       = (angle < M_PI / 2) ? (coordf_t) (tan_angle * layer_height) * wall_count :
    //                                                          std::numeric_limits<coordf_t>::max();
    coordf_t max_move_distance = 0;
    // const double   max_move_distance2      = max_move_distance * max_move_distance;
    double max_move_distance2 = 0;
    // const size_t   tip_layers = base_radius / layer_height;
    size_t tip_layers = 0;

    for (size_t layer_nr = contact_nodes.size() - 1; layer_nr > 0; layer_nr--) // Skip layer 0, since we can't drop down the vertices there.
    {
        coordf_t layer_height = layer_heights[layer_nr].height;
        if (old_layer_height != layer_height) {
            old_layer_height   = layer_height;
            max_move_distance  = (angle < M_PI / 2) ? (coordf_t) (tan_angle * layer_height) * wall_count :
                                                      std::numeric_limits<coordf_t>::max();
            max_move_distance2 = max_move_distance * max_move_distance;
            tip_layers         = base_radius / layer_height;
        }
        ...
        for (SupportNode* p_node : layer_contact_nodes) {
            ...
            if (support_on_buildplate_only && !node.to_buildplate) // Can't rest on model and unable to reach the build plate. Then we must
                                                                   // drop the node and leave parts unsupported.
            {
            ...
            tbb::parallel_for_each(nodes_vec.begin(), nodes_vec.end(), [&](const std::pair<const Point, SupportNode*>& entry) {
                ...
                if (node.type == ePolygon) {
                    ...
                    for (const Point& neighbour : neighbours) {
                        ...
                        if (is_inside_ex(node.overhang, neighbour) && is_inside_ex(node.overhang, pt_north) &&
                            ...
                            node.support_roof_layers_below = std::max(node.support_roof_layers_below,
                                                                      neighbour_node->support_roof_layers_below);
                ...
                for (size_t k = 0; k < iterations; k++) {
                    for (size_t i = 1; i < pts.size() - 1; i++) {
                        ...
                        if (k == iterations - 1) {
                            ...
                            BOOST_LOG_TRIVIAL(info) << "smooth_nodes: layer_nr=" << layer_nr << ", i=" << i << ", pt=" << pt
...
std::vector<LayerHeightData> TreeSupport::plan_layer_heights() // few- to see the implementation
{
        ...
        for (size_t layer_nr = range.begin(); layer_nr < range.end(); layer_nr++) {
            ...
            auto insert_point = [&](Point pt, const ExPolygon& overhang, double radius, bool force_add = false, bool add_interface = true) {
                ...
                if (force_add || !already_inserted.count(hash_pos)) {
                    ...
                    contact_node                = m_ts_data->create_node(pt, -gap_layers, layer_nr - 1, roof_layers + 1, to_buildplate,
                                                                         SupportNode::NO_PARENT, bottom_z, z_distance_top, 0, radius);
            ...
            for (const auto& overhang_part : layer->loverhangs) {
                ...
                if (m_support_params.support_style == smsTreeHybrid && overhang_part.area() > m_support_params.thresh_big_overhang &&
                    !is_sharp_tail) {
                    overhangs_regular = offset_ex(intersection_ex({overhang_part}, m_ts_data->m_layer_outlines_below[layer_nr - 1]),
                                                  radius_scaled);
