    ...
    std::function<void()>               throw_on_cancel)
{
    ...
    size_t num_buckets_initial;
    {
        ...
        // The bucket size used to be chosen by comparing the bucket count against
        // tbb::this_task_arena::max_concurrency(), purely to spread the first merge round over the
        // available threads. Influence-area merging is not associative, so that made the generated
        // organic supports depend on the core count of the machine doing the slicing: the same
        // project produced different supports on an 8-core and a 32-core host. Fix the bucket size
        // so the result is machine-independent; this is the branch every machine with few enough
        // cores already took.
