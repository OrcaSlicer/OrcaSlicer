...
std::string GCode::_extrude(const ExtrusionPath &path, std::string description, double speed)
{
    ...
    if (speed == -1) {
        if (path.role() == erPerimeter) {
            ...
            // Zone shell perimeters - use config or default to inner wall speed. The override is
            // object-wide; its fallback is per-nozzle like every other speed after the merge.
            speed = m_config.dual_infill_shell_speed.value;
            if (speed == 0)
                speed = NOZZLE_CONFIG(inner_wall_speed);
        } else if (path.role() == erZoneOuterInfill) {
            // Zone outer infill (the U-tube lattice) - config or default to sparse infill speed
            speed = m_config.dual_infill_outer_speed.value;
            if (speed == 0)
                speed = NOZZLE_CONFIG(sparse_infill_speed);
        } else if (path.role() == erZoneFloor) {
            // Zone floor (bottom of shell zone) - config or default to solid infill speed
            speed = m_config.dual_infill_floor_speed.value;
            if (speed == 0)
                speed = NOZZLE_CONFIG(internal_solid_infill_speed);
        } else if (path.role() == erZoneCeiling) {
            // Zone ceiling (top of shell zone) - config or default to top surface speed
