...
// Creality K2 SE firmware (2.3.6.73) picks its G-code parser from the "; generated by ..." header.
// On the exact spelling "OrcaSlicer" it selects a broken PrusaSlicer parser, whose metadata.py
// aborts with AttributeError: 'PrusaSlicer' object has no attribute 'parse_default_filament_colour'.
// virtual_sdcard.py then raises KeyError: 'metadata' and shuts down every MCU. Spelling the producer
// "Orca_Slicer" makes the firmware skip that parser and fall through to the "; generated by
// Creality_Print" marker already present in the profile's machine_start_gcode.
static bool needs_underscored_producer_header(const PrintConfig &config)
{
    return config.printer_model.value == "Creality K2 SE";
}
...
void GCode::_do_export(Print& print, GCodeOutputStream &file, ThumbnailsGeneratorCallback thumbnail_cb)
{
    ...
    if(!has_BTT_thumbnail){   
        ...
        std::string producer_header = Slic3r::header_slic3r_generated();
        if (needs_underscored_producer_header(print.config()))
            boost::replace_first(producer_header, "OrcaSlicer", "Orca_Slicer");
        file.write_format("; generated by %s on %s\n", producer_header.c_str(), Slic3r::Utils::local_timestamp().c_str());
        if (is_bbl_printers)
