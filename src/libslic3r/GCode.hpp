...
{
public:
    WipeTowerIntegration(const PrintConfig& print_config,
        ...
        , m_enable_wrapping_detection(print_config.enable_wrapping_detection && (print_config.wrapping_exclude_area.values.size() > 2) &&
                                      (slice_used_filaments.size() <= 1))
        , m_is_first_print(true)
        , m_print_config(&print_config)
        , m_last_wipe_tower_print_z(print_config.z_offset.value)
    {
        // initialize with the extruder offset of master extruder id
        m_extruder_offsets.resize(print_config.filament_map.size(),
                                  print_config.extruder_offset.get_at(print_config.master_extruder_id.value - 1));
...
public:
    ...
    void set_gcode_offset(double x, double y)
    {
        m_writer.set_xy_offset(x, y);
    ...
    std::string retract(bool toolchange         = false,
                        bool is_last_retraction = false,
                        LiftType lift_type      = LiftType::NormalLift,
                        bool apply_instantly    = false,
                        ExtrusionRole role      = erNone);
    std::string unretract(double extra_retract = 0.) { return m_writer.unlift() + m_writer.unretract(extra_retract); }
...
private:
    ...
    void process_layers(const Print& print,
    ...
    struct ObjectByExtruder
    {
        ...
        struct Island
        {
            struct Region
            {
                ...
                // std::vector<const ExtrusionEntity*> would be better here, but there is no way in C++ to convert from std::vector<T*>
                // std::vector<const T*> without copying.
                ...
                // Appends perimeter/infill entities and writes don't indices of those that are not to be extruder as part of
                // perimeter/infill wiping
            ...
            const std::vector<Region>& by_region_per_copy(std::vector<Region>& by_region_per_copy_cache,
                                                          ...
                                                          bool wiping_entities = false) const;
        };
    ...
    {
        InstanceToPrint(ObjectByExtruder& object_by_extruder,
                        size_t layer_id,
                        const PrintObject& print_object,
                        size_t instance_id,
                        size_t label_object_id)
            : object_by_extruder(object_by_extruder)
            , layer_id(layer_id)
        ...
        // Index into std::vector<LayerToPrint>, which contains Object and Support layers for the current print_z, collected for a single
        // object, or for possibly multiple objects with multiple instances.
    ...
    std::vector<InstanceToPrint> sort_print_object_instances(
        ...
        // Object and Support layers for the current print_z, collected for a single object, or for possibly multiple objects with multiple
        // instances.
        const std::vector<LayerToPrint>& layers,
    ...
    std::string extrude_perimeters(const Print& print,
...
#define ORCA_CHECK_GCODE_PLACEHOLDERS 1
    ...
    void _print_first_layer_extruder_temperatures(
        GCodeOutputStream& file, Print& print, const std::string& gcode, unsigned int first_printing_extruder_id, bool wait);
    ...
    friend ObjectByExtruder& object_by_extruder(std::map<unsigned int, std::vector<ObjectByExtruder>>& by_extruder,
                                                unsigned int extruder_id,
                                                size_t object_idx,
                                                size_t num_objects);
