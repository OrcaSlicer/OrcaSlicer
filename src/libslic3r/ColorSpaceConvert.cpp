...
double PivotRGB(double n) { return (n > 0.04045 ? std::pow((n + 0.055) / 1.055, 2.4) : n / 12.92) * 100.0; }
...
float DeltaE94(float l1, float a1, float b1, float l2, float a2, float b2)
{
    ...
    float delta_e94 = std::sqrt(std::pow(delta_l / (k_L * SL), 2) + std::pow(delta_c / (k_C * SC), 2) +
                                std::pow(delta_h / (k_H / k_C * SH), 2));
    return delta_e94;
}
