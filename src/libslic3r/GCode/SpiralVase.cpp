...
std::string SpiralVase::process_layer(const std::string &gcode, bool last_layer)
{
    ...
    const float min_segment_length = std::max(float(EPSILON), float(m_config.resolution.value));
    const float min_extrusion_length = 0.005f;
    ...
    m_reader.parse_buffer(gcode, [&new_gcode, &z, total_layer_length, layer_height, transition_in, &len, &current_layer, &previous_layer, &transition_gcode, transition_out, smooth_spiral, &max_xy_dist_for_smoothing, &last_point, starting_flowrate, finishing_flowrate, min_segment_length, min_extrusion_length]
        ...
        if (line.cmd_is("G1")) {
            ...
            if (line.extruding(reader) && line.dist_E(reader) < min_extrusion_length) return;
