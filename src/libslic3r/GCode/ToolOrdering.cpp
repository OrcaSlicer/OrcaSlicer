...
unsigned int LayerTools::wall_filament_id(const PrintRegion &region) const
{
	assert(region.config().wall_filament_id.value > 0);
	return ((this->extruder_override == 0) ? region.config().wall_filament_id.value : this->extruder_override) - 1;
}
...
unsigned int LayerTools::solid_infill_filament_id(const PrintRegion &region) const
{
	assert(region.config().solid_infill_filament_id.value > 0);
	return ((this->extruder_override == 0) ? region.config().solid_infill_filament_id.value : this->extruder_override) - 1;
}
    ...
    if (this->extruder_override == 0) {
        if (extrusions.has_infill()) {
            if (extrusions.has_solid_infill())
                extruder = region.config().solid_infill_filament_id;
            else
                extruder = region.config().sparse_infill_filament_id;
        } else
            extruder = region.config().wall_filament_id.value;
    } else
...
std::vector<unsigned int> ToolOrdering::generate_first_layer_tool_order(const Print& print)
{
    ...
    for (auto object : print.objects()) {
        ...
        for (auto layerm : target_layer->regions()) {
            int extruder_id = layerm->region().config().option("wall_filament_id")->getInt();
        ...
        for (const LayerRegion *layerm : layer->regions()) {
            ...
            if (! layerm->perimeters.entities.empty()) {
                ...
                if (something_nonoverriddable){
               		layer_tools.extruders.emplace_back((extruder_override == 0) ? region.config().wall_filament_id.value : extruder_override);
                    if (layerCount == 0) {
                        firstLayerExtruders.emplace_back((extruder_override == 0) ? region.config().wall_filament_id.value : extruder_override);
                    }
                }
            ...
            if (something_nonoverriddable || !m_print_config_ptr) {
            	if (extruder_override == 0) {
	                if (has_solid_infill)
	                    layer_tools.extruders.emplace_back(region.config().solid_infill_filament_id);
	                if (has_infill)
                ...
                if (is_infill_first != perimeters_done || wipe_into_infill_only) {
                    for (const ExtrusionEntity* ee : layerm->fills.entities) {                      // iterate through all infill Collections
                        ...
                        if (wipe_into_infill_only && ! is_infill_first)
                            ...
                            if (!lt.is_extruder_order(lt.wall_filament_id(region), new_extruder))
                                continue;
        ...
        for (size_t copy = 0; copy < num_of_copies; ++copy) {    // iterate through copies first, so that we mark neighbouring infills to minimize travel moves
            for (const LayerRegion *layerm : this_layer->regions()) {
                ...
                for (const ExtrusionEntity* ee : layerm->fills.entities) {                      // iterate through all infill Collections
                    ...
                    || lt.is_extruder_order(lt.wall_filament_id(region), last_nonsoluble_extruder    // !infill_first, but perimeter is already printed when last extruder prints
