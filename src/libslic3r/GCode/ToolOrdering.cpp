...
// ============================================================================
// GroupReorder namespace: standalone context builder for dynamic reorder
// Ported from BBL to enable per-combo-range filament-to-nozzle optimization.
// ============================================================================
...
// Mirrors the inline logic in get_recommended_filament_maps (L1237-1278).
...
// Re-assigns nozzle slots to minimize flush cost given the current nozzle state
// (which filament is already loaded in each nozzle). Processes each extruder
// independently, building a cost matrix and solving via MinFlushFlowSolver.
...
//
// Groups contiguous layers that use the same filament combo (sorted set of
// filament IDs). For each combo range, independently computes optimal
// filament-to-nozzle assignment via get_recommended_filament_maps, refines
// it with refine_groups_by_Nozzle_State (min-cost flow), then reorders
// filaments within each nozzle group. Tracks nozzle state across ranges
// via io_nozzle_status so that subsequent ranges benefit from the loaded
// filament in each nozzle slot.
//
// This is the key optimization that BBL uses to reduce flush volume by ~30%
// compared to a single global filament-to-nozzle assignment.
//
...
void ToolOrdering::reorder_extruders_for_minimum_flush_volume(bool reorder_first_layer)
{
    ...
    if(support_multi_nozzle && m_print->get_layered_nozzle_group_result()){
        ...
        // When enabled, replaces the single-pass filament-to-nozzle assignment
        // with per-combo-range planning that tracks nozzle state across ranges.
