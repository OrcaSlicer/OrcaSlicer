...
constexpr float wt2_flat_iron_speed                    = 10.f * 60.f;
static const double wt2_wipe_tower_wall_infill_overlap = 0.0;
static constexpr double wt2_WIPE_TOWER_RESOLUTION      = 0.1;
static constexpr double WT_SIMPLIFY_TOLERANCE_SCALED2  = 0.001f / SCALING_FACTOR_INTERNAL;
static constexpr int wt2_arc_fit_size                  = 20;
#define SCALED_WIPE_TOWER_RESOLUTION2 (wt2_WIPE_TOWER_RESOLUTION / SCALING_FACTOR_INTERNAL)
enum class wt2_LimitFlow { None, LimitPrintFlow, LimitRammingFlow };
static const std::map<float, float> wt2_nozzle_diameter_to_nozzle_change_width{{0.2f, 0.5f}, {0.4f, 1.0f}, {0.6f, 1.2f}, {0.8f, 1.4f}};

inline float wt2_align_round(float value, float base) { return std::round(value / base) * base; }

inline float wt2_align_ceil(float value, float base) { return std::ceil(value / base) * base; }

inline float wt2_align_floor(float value, float base) { return std::floor((value) / base) * base; }

static bool wt2_is_valid_gcode(const std::string& gcode)
{
...
static Polygon wt2_chamfer_polygon(Polygon& polygon, double chamfer_dis = 2., double angle_tol = 30. / 180. * PI)
{
...
static Polygon wt2_rounding_rectangle(Polygon& polygon, double rounding = 2., double angle_tol = 30. / 180. * PI)
{
    if (polygon.points.size() < 3)
        return polygon;
    Polygon res;
...
static Polygon wt2_scale_polygon(const std::vector<Vec2f>& points)
{
    Polygon res;
...
static std::vector<Vec2f> wt2_unscale_polygon(const Polygon& polygon)
{
...
static Polygon wt2_generate_rectange(const Line& line, coord_t offset)
{
    Point p1 = line.a;
    Point p2 = line.b;
...
struct wt2_Segment
{
    ...
    wt2_Segment(const Vec2f& s, const Vec2f& e) : start(s), end(e) {}
    bool is_valid() const { return start.y() < end.y(); }
};

static Polygon wt2_generate_rectange_polygon(const Vec2f& wt_box_min, const Vec2f& wt_box_max)
{
    Polygon res;
...
public:
    ...
    WipeTowerWriter2& extrude_arc_explicit(ArcSegment& arc,
                                           ...
                                           wt2_LimitFlow limit_flow = wt2_LimitFlow::LimitPrintFlow)
    {
       ...
       // This is left zero if it is a travel move.
        ...
        if (f != 0.f && f != m_current_feedrate) {
            if (limit_flow != wt2_LimitFlow::None) {
                float e_speed = e / (((len == 0.f) ? std::abs(e) : len) / f * 60.f);
                float tmp     = m_filpar[m_current_tool].max_e_speed;
                // if (limit_flow == wt2_LimitFlow::LimitRammingFlow)
    ...
    WipeTowerWriter2& extrude_arc(ArcSegment& arc, float f = 0.f, wt2_LimitFlow limit_flow = wt2_LimitFlow::LimitPrintFlow)
    ...
    {
        auto get_closet_idx = [this](std::vector<wt2_Segment>& corners) -> int {
        ...
        std::vector<wt2_Segment> segments;
        if (m_enable_arc_fitting) {
            for (auto& pl : pls)
                pl.simplify_by_fitting_arc(SCALED_WIPE_TOWER_RESOLUTION2);
            ...
            for (auto& pl : pls)
                pl.simplify(SCALED_WIPE_TOWER_RESOLUTION2);
...
WipeTower::ToolChangeResult WipeTower2::construct_tcr(
    WipeTowerWriter2& writer, bool priming, size_t old_tool, bool is_finish, bool is_contact) const
{
    ...
    bool /*last_wipe_inside_wipe_tower*/)
{
    ...
    for (size_t idx_tool = 0; idx_tool < tools.size(); ++idx_tool) {
        ...
        toolchange_Change(writer, tool, m_filpar[tool].material, m_filpar[tool].first_layer_temperature,
                          false);              // Select the tool, set a speed override for soluble and flex materials.
...
WipeTower::ToolChangeResult WipeTower2::tool_change(size_t tool)
{
    ...
    if (tool != (unsigned int) -1) { // This is not the last change.
        ...
        int wait_for_temp = interface_layer && m_filpar[tool].interface_print_temperature > 0 ?
                                ...
                                (is_first_layer() || m_filpar[tool].temperature == 0 ? m_filpar[tool].first_layer_temperature :
                                                                                       m_filpar[tool].temperature);
        toolchange_Change(writer, tool, m_filpar[tool].material, wait_for_temp,
                          true); // Change the tool, set a speed override for soluble and flex materials.
        ...
        toolchange_Wipe(writer, cleaning_box, wipe_volume, interface_layer, false,
                        fill_box); // Wipe the newly loaded filament until the end of the assigned wipe area.
        if (interface_layer) {
            int interface_temp = m_filpar[tool].interface_print_temperature;
            if (!m_enable_tower_interface_cooldown_during_tower && interface_temp > 0 && interface_temp != base_temp)
                writer.set_extruder_temp(base_temp, false);
                                   ...
                                   const int new_temperature)
{
    ...
    if (boundary_wipe_start && m_layer_info != m_plan.end())
        for (const auto& tch : m_layer_info->tool_changes)
            if (tch.old_tool == m_current_tool) {
                planned_ramming_depth = tch.ramming_depth;
                break;
            }
    ...
    if (planned_ramming_depth > 0.f) {
        ...
        const float fill_feed = m_filpar[m_current_tool].ramming_speed.empty() ?
                                    3000.f :
    ...
    if (planned_ramming_depth > 0.f) {
        ...
        writer.travel(Vec2f(ramming_start_pos.x(), cleaning_box.ld.y() + m_depth_traversed +
                                                       wipe_start_offset_after_ram(planned_ramming_depth, is_first_layer()) +
                                                       m_perimeter_width),
                      2400.f);
        m_left_to_right = true;
    } else if (do_ramming)
        ...
        writer.set_position(Vec2f(end_of_ramming.x(), cleaning_box.ld.y() + m_depth_traversed +
                                                          wipe_start_offset_after_ram(0.f, is_first_layer()) + m_perimeter_width));
    }
...
void WipeTower2::toolchange_Change(
    WipeTowerWriter2& writer, const size_t new_tool, const std::string& new_material, const int wait_for_temp, const bool wait_beside_tower)
{
    ...
    if (wait_for_temp_here && wait_beside_tower) {
        ...
        if (m_wall_type == (int) wtwRib) {
            ...
            const double support_scale =
                get_wipe_tower_cone_base(m_wipe_tower_width, m_wipe_tower_height, m_wipe_tower_depth, m_wipe_tower_cone_angle).second;
        ...
        if (have_park) {
            ...
            writer.feedrate(m_travel_speed * 60.f)
                .append(std::string("G1 X") + Slic3r::float_to_string_decimal_point(stop.x()) + " Y" +
                        Slic3r::float_to_string_decimal_point(stop.y()) + never_skip_tag() + "\n");
        }
...
void WipeTower2::toolchange_Wipe(WipeTowerWriter2& writer,
                                 const WipeTower::box_coordinates& cleaning_box,
                                 float wipe_volume,
                                 ...
                                 bool fill_box)
{
    ...
    float dy        = wipe_row_spacing(
        is_first_layer()); // Don't use the extra spacing for the first layer, but do use the spacing resulting from increased flow.
    ...
    for (int i = 0; true; ++i) {
        ...
        if (i == 0 && m_use_gap_wall && !interface_layer && !priming && m_left_to_right && writer.x() - xl < 2.5f * line_width) {
            ...
            writer.spiral_flat_ironing(writer.pos(), m_filpar[m_current_tool].tower_ironing_area, m_perimeter_width, wt2_flat_iron_speed);
...
WipeTower2::WipeTowerInfo::ToolChange WipeTower2::set_toolchange(
    size_t old_tool, size_t new_tool, float layer_height, float wipe_volume, bool first_layer_plan)
{
    float width             = m_wipe_tower_width - 3 * m_perimeter_width;
    float length_to_extrude = volume_to_length((m_semm ? 0.25f : m_filpar[old_tool].multitool_ramming_time) *
                                                   std::accumulate(m_filpar[old_tool].ramming_speed.begin(),
    ...
    float ramming_depth            = do_ramming ? ((int(length_to_extrude / width) + 1) *
    ...
    float first_wipe_line = (do_ramming && !boundary_wipe_start) ?
                                -(width * ((length_to_extrude / width) - int(length_to_extrude / width)) - width) :
                                0.f;
    ...
    float wiping_depth = get_wipe_depth(wipe_volume - first_wipe_volume, layer_height, m_perimeter_width, m_extra_flow, planning_spacing,
                                        width);
...
void WipeTower2::save_on_last_wipe()
{
    for (m_layer_info = m_plan.begin(); m_layer_info < m_plan.end(); ++m_layer_info) {
        ...
        for (int i = 0; i < int(m_layer_info->tool_changes.size()); ++i) {
            ...
            if (i == idx) {
                recompute_toolchange(toolchange, length_to_volume(finish_layer().total_extrusion_length_in_plane(), m_perimeter_width,
                                                                  m_layer_info->height));
...
int WipeTower2::first_toolchange_to_nonsoluble_nonsupport(const std::vector<WipeTowerInfo::ToolChange>& tool_changes) const
{
    ...
    auto is_wall_filament = [this](size_t tool) { return !m_filpar[tool].is_soluble && !m_filpar[tool].is_support; };
    for (size_t idx = 0; idx < tool_changes.size(); ++idx)
...
void WipeTower2::generate(std::vector<std::vector<WipeTower::ToolChangeResult>>& result)
{
    ...
    if (m_wall_type == (int) wtwRib) {
        ...
        if (max_depth > EPSILON) {
            m_wipe_tower_width = wt2_align_ceil(std::sqrt(max_depth * m_wipe_tower_width), m_perimeter_width);
...
Polygon WipeTower2::generate_rib_polygon(const WipeTower::box_coordinates& wt_box)
{
    ...
    Polygon poly_1 = wt2_generate_rectange(line_1, diagonal_width);
    Polygon poly_2 = wt2_generate_rectange(line_2, diagonal_width);
    Polygon poly;
                                              ...
                                              bool extrude_perimeter)
{
    ...
    Polygon wall_polygon = rib_wall ? generate_rib_polygon(wt_box) : wt2_generate_rectange_polygon(wt_box.ld, wt_box.ru);
    ...
    if (m_used_fillet) {
        if (!rib_wall && m_y_shift > EPSILON) // do nothing because the fillet will cause it to be suspended.
        {
            ...
            Polygon wt_box_polygon = wt2_generate_rectange_polygon(wt_box.ld, wt_box.ru);
    ...
    if (m_use_gap_wall) {
        ...
        const std::vector<Vec2f>& layer_skip_points = layer_id < m_wall_skip_points.size() ? m_wall_skip_points[layer_id] : no_skip_points;
