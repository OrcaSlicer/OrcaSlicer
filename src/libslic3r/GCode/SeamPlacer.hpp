...
public:
  ...
  // Returns true when an outer or inner seam uses a shared painted-boundary anchor.
  bool place_seam(const Layer *layer, ExtrusionLoop &loop, const Point &last_pos, float& overhang,
                  bool prefer_color_boundary = false) const;
private:
