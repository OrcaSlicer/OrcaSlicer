...
std::string FanMover::_set_fan(int16_t speed) {
    ...
    const int floor_pct = m_writer.config.pct_fan_min_pwm.value;
    const unsigned int pct_fan_min_pwm = floor_pct > 0 ? static_cast<unsigned int>(floor_pct) : 0u;
    return GCodeWriter::set_fan(m_writer.config.gcode_flavor.value, speed, pct_fan_min_pwm);
}
