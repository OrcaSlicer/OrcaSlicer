...
public:
    ...
    // True when this region has a dual-infill zone ON THIS LAYER.
    //
    // `dual_infill_enabled` alone is NOT that test, and using it as one was a real bug: nothing
    // re-types internal surface on a layer with no zone boundary, so `stInternal &&
    // dual_infill_enabled` classified the ENTIRE cross-section as outer zone -- lattice pattern,
    // zone speeds and zone filament applied to the whole interior. The layer's own
    // `zone_boundary` is the fact that matters, and it is already computed.
    //
    // Defined once because the condition had been written out seven times with only one copy
    // (the zone floor/ceiling detector) getting it right; seven copies of one test is how they
    // diverged.
    bool                        has_dual_infill_zone() const;
