        ...
        bool* compatibility, DynamicPrintConfig* compatibility_payload)
{
    ...
    if (boost::algorithm::iends_with(input_file, ".3mf")) {
        ...
        if (prusa_file_parser.check_3mf_from_prusa(input_file)) {
            ...
            // BBS: add part plate related logic
            // BBS: backup & restore
            result = load_bbs_3mf(input_file.c_str(), config, config_substitutions, &model, plate_data, project_presets, &is_bbl_3mf, &is_orca_3mf, file_version, proFn, options, project, 0, compatibility, compatibility_payload);
        }
    }
