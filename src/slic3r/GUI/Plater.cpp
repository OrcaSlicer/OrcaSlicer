...
std::map<int, DynamicPrintConfig> Sidebar::build_filament_ams_list(MachineObject* obj)
{
    ...
    auto build_tray_config = [](DevAmsTray const &tray, std::string const &name, std::string ams_id, std::string slot_id) {
        BOOST_LOG_TRIVIAL(info) << boost::format("build_filament_ams_list: name %1% setting_id %2% type %3% vendor %4% color %5%")
                    % name % tray.setting_id % tray.m_fila_type % tray.sub_brands % tray.color;
        ...
        tray_config.set_key_value("filament_vendor", new ConfigOptionStrings{tray.sub_brands});
