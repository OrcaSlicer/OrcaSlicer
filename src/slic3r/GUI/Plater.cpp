...
template<typename T, typename OptionType> static void set_config_values(DynamicPrintConfig* config, const std::string& key, T value)
{
...
void Plater::mark_plate_toolbar_image_dirty() { m_b_plate_toolbar_image_dirty = true; }

bool Plater::is_plate_toolbar_image_dirty() const { return m_b_plate_toolbar_image_dirty; }

void Plater::clear_plate_toolbar_image_dirty() { m_b_plate_toolbar_image_dirty = false; }
...
wxString sanitize_window_layout_for_wayland(const wxString& layout, bool* removed_floating_state = nullptr)
{
    ...
    constexpr unsigned int disabled_wayland_flags = static_cast<unsigned int>(wxAuiPaneInfo::optionFloating) |
...
struct ExtruderGroup : StaticGroup
{
    ...
    {
        if (hover_label)
            hover_label->SetOnHoverClick(std::move(on_click));
    }
...
void Sidebar::priv::layout_printer(bool isBBL, bool isDual)
{
    ...
    if (vsizer_printer->GetItemCount() == 0) {
        ...
        if (!extruder_separator_icon) {
            ...
            extruder_separator_icon = new wxStaticBitmap(m_panel_printer_content, wxID_ANY, bitmap.bmp(), wxDefaultPosition,
                                                         bitmap.GetBmpSize());
...
ExtruderGroup::ExtruderGroup(wxWindow* parent, int index, wxString const& title) : StaticGroup(parent, wxID_ANY, wxString())
{
...
bool Sidebar::priv::switch_diameter(bool single)
{
    ...
    if (single) {
        ...
        if (diameter_left != diameter_right) {
            ...
            auto left_name = _L(DevPrinterConfigUtil::get_toolhead_display_name(printer_type, DEPUTY_EXTRUDER_ID, ToolHeadComponent::Nozzle,
                                                                                ToolHeadNameCase::SentenceCase));
            auto right_name = _L(DevPrinterConfigUtil::get_toolhead_display_name(printer_type, MAIN_EXTRUDER_ID, ToolHeadComponent::Nozzle,
                                                                                 ToolHeadNameCase::SentenceCase));
...
static bool is_same_nozzle_config(const std::map<int, std::vector<DevNozzle>>& config1, const std::map<int, std::vector<DevNozzle>>& config2)
{
    ...
    for (const auto& [eid, nozzles1] : config1) {
        ...
        if (nozzles1.size() != nozzles2.size())
            return false;
...
std::optional<NozzleOption> Sidebar::priv::get_nozzle_options(MachineObject* obj,
                                                              int extruder_count,
        ...
        if (!saved_dev_id.empty() && !saved_nozzle_config_str.empty() && !saved_nozzle_option_str.empty() && saved_dev_id == curr_dev_id) {
            ...
            if (can_reuse_saved_option) {
                ...
                if (nozzle_volume_type_opt) {
                    for (int extruder_id = 0; extruder_id < extruder_count && extruder_id < (int) nozzle_volume_type_opt->values.size();
                         ++extruder_id)
...
bool Sidebar::priv::sync_extruder_list(bool& only_external_material, bool is_manual)
{
    ...
    // abort the sync.
    const ConfigOptionIntsNullable* extruder_max_nozzle_count = cur_preset.config.option<ConfigOptionIntsNullable>(
    ...
    for (size_t index = 0; index < extruder_nums; ++index) {
        ...
        nozzle_diameters[extruder_id] = nozzle_option ? atof(nozzle_option->diameter.c_str()) :
                                                        obj->GetExtderSystem()->GetNozzleDiameter(index);
...
Sidebar::Sidebar(Plater* parent) : wxPanel(parent, wxID_ANY, wxDefaultPosition, wxSize(39 * wxGetApp().em_unit(), -1)), p(new priv(parent))
{
    ...
    // add printer
    {
        ...
        p->m_panel_printer_separator = new wxPanel(p->scrolled, wxID_ANY, wxDefaultPosition,
                                                   wxSize(-1, FromDIP(2))); // ORCA staticline class not works without string
...

    {
        ...
        p->m_panel_filament_title->Bind(wxEVT_LEFT_UP, [this](wxMouseEvent& e) {
            if (!p || !p->m_panel_filament_content || !m_scrolled_sizer || !p->m_bpButton_set_filament || !p->m_purge_mode_btn ||
                !p->m_flushing_volume_btn || !p->m_bpButton_add_filament || !ams_btn)
                return;
            // ORCA exclude area of del button from titlebar collapse/expand feature to fix undesired collapse when user spams del filament
            // button also block fold/unfold feature when user clicks to spacing between icons
            int exclude_pt = p->m_bpButton_set_filament->GetPosition().x; // maximum fixed item
            if (p->m_purge_mode_btn->IsShown())
                exclude_pt = p->m_purge_mode_btn->GetPosition().x;
            else if (p->m_flushing_volume_btn->IsShown())
                exclude_pt = p->m_flushing_volume_btn->GetPosition().x;
            else if (p->m_bpButton_add_filament->IsShown())
                exclude_pt = p->m_bpButton_add_filament->GetPosition().x - FromDIP(30); // reserve spacing for delete button
            else if (ams_btn->IsShown())
                exclude_pt = ams_btn->GetPosition().x;
        ...
        // ORCA add bottom border for seperation wile sections folded
        p->m_panel_filament_separator = new wxPanel(p->scrolled, wxID_ANY, wxDefaultPosition,
                                                    wxSize(-1, FromDIP(2))); // ORCA staticline class not works without string
        ...
        p->m_purge_mode_btn->Bind(wxEVT_BUTTON, [](wxCommandEvent& e) {
            ...
            auto support_fast_purge_opt = preset_bundle.printers.get_edited_preset().config.option<ConfigOptionBool>(
            ...
            if (dlg.ShowModal() == wxID_OK) {
                preset_bundle.project_config.set_key_value("prime_volume_mode",
...
void Sidebar::init_filament_combo(PlaterPresetComboBox** combo, const int filament_idx)
{
    *combo = new PlaterPresetComboBox(p->m_panel_filament_content, Slic3r::Preset::TYPE_FILAMENT);
    ...
    combo_and_btn_sizer->Add(*combo, 1, wxALL | wxEXPAND, FromDIP(2))
        ->SetMinSize({-1, 30 * wxGetApp().em_unit() / 10}); // ORCA ensure height matches with PlaterPresetComboBox
...
void Sidebar::update_presets(Preset::Type preset_type)
{
    ...
    case Preset::TYPE_PRINTER: {
        ...
        if (is_dual_extruder) {
            ...
            auto left_title  = DevPrinterConfigUtil::get_toolhead_display_name(printer_type, DEPUTY_EXTRUDER_ID, ToolHeadComponent::Nozzle,
                                                                               ToolHeadNameCase::TitleCase);
            auto right_title = DevPrinterConfigUtil::get_toolhead_display_name(printer_type, MAIN_EXTRUDER_ID, ToolHeadComponent::Nozzle,
                                                                               ToolHeadNameCase::TitleCase);
        ...
        auto update_extruder_variant   = [printer_model, extruders_def, extruders, nozzle_volumes_def, nozzle_volumes, extruder_variants,
                                          diameter, extruder_max_nozzle_count](ExtruderGroup& extruder, int index) {
            ...
            for (size_t i = 0; i < nozzle_volumes_def->enum_labels.size(); ++i) {
                ...
                if (boost::algorithm::contains(extruder_variants->values[index], type + " " + nozzle_volumes_def->enum_labels[i]) ||
                    ...
                    if (nozzle_volumes_def->enum_keys_map->at(nozzle_volumes_def->enum_values[i]) == NozzleVolumeType::nvtHighFlow &&
                        (diameter == "0.2" || is_skip_high_flow_printer(printer_model)))
                        continue;
        ...
        if (is_dual_extruder) {
            std::string printer_type = printer_preset.get_printer_type(wxGetApp().preset_bundle);
            p->left_extruder->SetTitle(
                _L(DevPrinterConfigUtil::get_toolhead_display_name(printer_type, DEPUTY_EXTRUDER_ID, ToolHeadComponent::Nozzle,
                                                                   ToolHeadNameCase::TitleCase)));
            p->right_extruder->SetTitle(
                _L(DevPrinterConfigUtil::get_toolhead_display_name(printer_type, MAIN_EXTRUDER_ID, ToolHeadComponent::Nozzle,
                                                                   ToolHeadNameCase::TitleCase)));
...
bool Sidebar::is_fila_switch_ready() { return p->is_fila_switch_ready(); }

void Sidebar::reset_fila_switch() { p->update_extruder_separator_icon(false, false); }
...
void Sidebar::update_dynamic_filament_list() { dynamic_filament_list.update(); }
...
Plater::priv::priv(Plater* q, MainFrame* main_frame)
    ...
    , config(Slic3r::DynamicPrintConfig::new_from_defaults_keys(
          {"printable_area", "bed_exclude_area", "wrapping_exclude_area", "extruder_printable_area", "bed_custom_texture",
           ...
           "skirt_distance", "skirt_start_angle", "brim_width", "brim_object_gap", "brim_flow_ratio", "brim_use_efc_outline",
           "combine_brims", "brim_type", "nozzle_diameter", "single_extruder_multi_material", "preferred_orientation", "enable_prime_tower",
           "wipe_tower_x", "wipe_tower_y", "prime_tower_width", "prime_tower_brim_width", "prime_tower_skip_points",
           "prime_tower_enable_framework", "prime_tower_infill_gap", "prime_volume", "extruder_colour", "filament_colour", "filament_type",
           "material_colour", "printable_height", "extruder_printable_height", "printer_model", "printer_technology",
           // These values are necessary to construct SlicingParameters by the Canvas3D variable layer height editor.
           "layer_height", "initial_layer_print_height", "min_layer_height", "max_layer_height", "wall_loops", "outer_wall_filament_id",
           "inner_wall_filament_id", "sparse_infill_density", "sparse_infill_filament_id", "top_shell_layers", "enable_support",
           "support_filament", "support_interface_filament", "support_top_z_distance", "support_bottom_z_distance", "raft_layers",
           "wipe_tower_rotation_angle", "wipe_tower_cone_angle", "wipe_tower_extra_spacing", "wipe_tower_extra_flow",
           "wipe_tower_max_purge_speed", "wipe_tower_wall_type", "wipe_tower_extra_rib_length", "wipe_tower_rib_width",
           "wipe_tower_fillet_wall", "wipe_tower_filament", "best_object_pos", "master_extruder_id"}))
    ...
    , partplate_list(this->q, &model)
{
    ...
    m_aui_mgr.AddPane(sidebar, wxAuiPaneInfo().Name("sidebar").Left().CloseButton(false).TopDockable(false).BottomDockable(false).BestSize(
                                   wxSize(39 * wxGetApp().em_unit(), 90 * wxGetApp().em_unit())));
    ...
    if (wxGetApp().is_editor()) {
        ...
        this->q->Bind(EVT_RESTORE_PROJECT, [this, last = last_backup](wxCommandEvent& e) {
            ...
            if (Slic3r::has_restore_data(last_backup, originfile)) {
                auto result = MessageDialog(this->q, _L("Previously unsaved items have been detected. Do you want to restore them\?"),
...
wxString Plater::get_slice_warning_string(GCodeProcessorResult::SliceWarning& warning)
{
    if (warning.msg == BED_TEMP_TOO_HIGH_THAN_FILAMENT) {
        return _L("The current heatbed temperature is relatively high. The nozzle may clog when printing this filament in a closed "
        ...
        return _L("The nozzle hardness required by the filament is higher than the default nozzle hardness of the printer. Please replace "
                  "the hardened nozzle or filament, otherwise, the nozzle will be worn down or damaged.");
...
std::vector<size_t> Plater::priv::load_files(const std::vector<fs::path>& input_files, LoadStrategy strategy, bool ask_multi)
{
                ...
                En3mfType en_3mf_file_type = En3mfType::From_BBS;
                {
                    ...
                    BOOST_LOG_TRIVIAL(info)
                        ...
                        << boost::format(", plate_data.size %1%, project_preset.size %2%, is_bbs_or_orca_3mf %3%, file_version %4% \n") %
                               plate_data.size() % project_presets.size() %
                    ...
                    const wxString msg_old_orca_geometry    = _L(
                        "The 3MF file was generated by an old OrcaSlicer version, loading geometry data only.");
                    ...
                    if (en_3mf_file_type == En3mfType::From_Prusa) {
                        ...
                        if (load_config && (file_version < app_version) && file_version == Semver("2.3.1-alpha")) {
                            if (!config_loaded.opt_string("sparse_infill_rotate_template").empty()) {
                                ...
                                if (!is_safe_to_rotate) {
                                    ...
                                    msg_text += "\n\n" + _(L("Would you like OrcaSlicer to automatically fix this by clearing the rotation "
                                                             "template settings?"));
                        ...
                        if (file_version <= orca_tag_start_version) {
                            ...
                            if (load_config && (file_version == Semver("2.3.1-alpha"))) {
                                if (!config_loaded.opt_string("sparse_infill_rotate_template").empty()) {
                                    ...
                                    bool is_safe_to_rotate = _sparse_infill_pattern == ipRectilinear || _sparse_infill_pattern == ipLine ||
                                                             ...
                                                             _sparse_infill_pattern == ipLockedZag;
                                    if (!is_safe_to_rotate) {
                                        wxString msg_text = _(L("This project was created with an OrcaSlicer 2.3.1-alpha and uses "
                                                                "infill rotation template settings that may not work properly with your "
                                                                "current infill pattern. "
                                                                "This could result in weak support or print quality issues."));
                                        msg_text += "\n\n" + _(L("Would you like OrcaSlicer to automatically fix this by clearing the "
                                                                 "rotation template settings?"));
                                        MessageDialog dialog(wxGetApp().plater(), msg_text, "", wxICON_WARNING | wxYES | wxNO);
                                        dialog.SetButtonLabel(wxID_YES, _L("Yes"));
                                        dialog.SetButtonLabel(wxID_NO, _L("No"));
                                        if (dialog.ShowModal() == wxID_YES) {
                            ...
                            if (load_config && config_loaded.empty()) {
                                ...
                                if (config_substitutions.unrecogized_keys.size() > 0) {
                                    wxString text = wxString::Format(_L("The 3MF was created by BambuStudio (version %s), which is newer "
                                                                        "than the compatible version %s. Found unrecognized settings:"),
                                    ...
                                    wxString text =
                                        wxString::Format(_L("The 3MF was created by BambuStudio (version %s), which is newer than the "
                    ...
                    if (plate_data.size() > 0) {
                        ...
                        if ((en_3mf_file_type == En3mfType::From_BBS || en_3mf_file_type == En3mfType::From_Orca) &&
                            (file_version < old_version) && load_model && load_config && !config_loaded.empty()) {
                            translate_old = true;
                    ...
                    if (load_config && !config_loaded.empty()) {
                        ...
                        if (!validity.empty()) {
                            BOOST_LOG_TRIVIAL(info)
                                << __FUNCTION__ << ":" << __LINE__ << " " << boost::format("Param values in 3mf error: ");
                ...
                if (load_config) {
                    if (!config.empty()) {
                        ...
                        // always load config
                        {
                            ...
                            if (auto* nozzle_volumes = wxGetApp().preset_bundle->project_config.option<ConfigOptionEnumsGeneric>(
                                    "nozzle_volume_type")) {
                                const int extruder_count = wxGetApp().preset_bundle->get_printer_extruder_count();
                                for (int extruder_id = 0; extruder_id < extruder_count && extruder_id < (int) nozzle_volumes->values.size();
                                     ++extruder_id)
                                    updateNozzleCountDisplay(wxGetApp().preset_bundle, extruder_id,
                                                             NozzleVolumeType(nozzle_volumes->values[extruder_id]));
                            }
                        }
                    }
                    if (!silence)
    ...
    if (new_model != nullptr && new_model->objects.size() > 1) {
        ...
        // DONE when manually split to object ask users if looks_like_multipart and none have auto_drob disabled if they want to disable
        // auto_drop for all resulting objects.
...
void Plater::priv::process_validation_warning(StringObjectException const& warning) const
{
    if (warning.string.empty())
        ...
        auto action_fn = (mo || mi || !warning.opt_key.empty()) ?
                             [id = mo ? mo->id() : (mi ? mi->id() : 0), parent_id = mi ? mi->get_object()->id() : 0,
                              is_inst = (mi != nullptr), opt = warning.opt_key](wxEvtHandler*) {
                                 auto& objects = wxGetApp().model().objects;

                                 if (is_inst) {
                                     bool selected = false;
                                     auto iter     = std::find_if(objects.begin(), objects.end(),
                                                                  [parent_id](auto o) { return o->id() == parent_id; });
                                     if (iter != objects.end()) {
        ...
        if (mi)
            hypertext += std::string(" [") + mi->get_object()->name + "]";
...
bool Plater::priv::replace_volume_with_stl(int object_idx, int volume_idx, const fs::path& new_path, const std::string& snapshot)
{
    ...
    if (new_model.objects.size() > 1 || new_model.objects.front()->volumes.size() > 1) {
        MessageDialog dlg(q, _L("Unable to replace with more than one volume"), _L("Error during replacement"),
...
void Plater::priv::replace_with_stl()
{
    ...
    if (out_path.empty()) {
        MessageDialog dlg(q, _L("File for the replacement wasn\'t selected"), _L("Error during replacement"),
                          wxOK | wxOK_DEFAULT | wxICON_WARNING);
        dlg.ShowModal();
        return;
    }
...
void Plater::priv::replace_all_with_stl()
{
    ...
    fs::path out_path = dialog.GetPath().ToUTF8().data();
    if (out_path.empty()) {
        MessageDialog dlg(q, _L("Directory for the replace wasn't selected"), _L("Error during replacement"),
                          wxOK | wxOK_DEFAULT | wxICON_WARNING);
        dlg.ShowModal();
        return;
    }
            ...
            if (has_source || has_name) {
                ...
                if (has_source && old_volume->source.object_idx < int(new_model.objects.size())) {
                    ...
                    if (old_volume->source.volume_idx < int(obj->volumes.size())) {
                        ...
                        if (new_input_file == old_input_file || boost::algorithm::iequals(fs::path(new_input_file).filename().string(),
            ...
            if (Slic3r::DeviceManager* dev = Slic3r::GUI::wxGetApp().getDeviceManager()) {
                ...
                if (obj && obj->is_multi_extruders()) {
                    ...
                    if (cur_preset.get_printer_type(preset_bundle) == obj->get_show_printer_type()) {
                        ...
                        const auto& extruders = obj->GetExtderSystem()->GetExtruders();
                        for (const DevExtder& extruder : extruders) {
                            if (!obj->GetExtderSystem()->NozzleDiameterMatchesOrUnknown(extruder.GetExtId(),
                                                                                        float(preset_nozzle_diameter))) {
...
void Plater::priv::on_tab_selection_changing(wxBookCtrlEvent& e)
{
    ...
    const bool use_printer_agents    = wxGetApp().app_config->get_bool("use_printer_agents");
    const bool use_native_device_tab = wxGetApp().preset_bundle && (wxGetApp().preset_bundle->use_bbl_device_tab() || use_printer_agents);
    ...
    if (result == wxID_OK) {
        ...
        if (wxGetApp().install_network_plugin_from_ota(had_cache)) {
            ...
            if (wxGetApp().hot_reload_network_plugin()) {
                MessageDialog dlg_ok(wxGetApp().mainframe, _L("Network plug-in switched successfully."), _L("Success"),
                                     wxOK | wxICON_INFORMATION);
                dlg_ok.ShowModal();
            } else {
                MessageDialog dlg_fail(wxGetApp().mainframe, _L("Failed to load network plug-in. Please restart the application."),
...
bool Plater::priv::check_ams_status_impl(bool is_slice_all)
{
    ...
    if (preset_bundle && preset_bundle->printers.get_edited_preset().get_printer_type(preset_bundle) == obj->get_show_printer_type()) {
        ...
        if (obj->GetExtderSystem()->GetTotalExtderCount() == 2 && nozzle_volumes_values.size() == 2) {
            ...
            auto nozzle_diameter_values =
                preset_bundle->printers.get_edited_preset().config.option<ConfigOptionFloatsNullable>("nozzle_diameter")->values;
            ...
            for (const auto& preset_groups : preset_nozzle_infos) {
                for (const auto& preset_group : preset_groups) {
                    if (preset_group.nozzle_count == 0) {
                        ...
                        if (std::find_if(printer_groups.begin(), printer_groups.end(), [&preset_group](const NozzleGroupInfo& elem) {
                                return preset_group.is_same_type(elem);
                            }) != printer_groups.end()) {
...
int Plater::new_project(bool skip_confirm, bool silent, const wxString& project_name)
{
    ...
    auto check = [this, &transfer_preset_changes](bool yes_or_no) {
        ...
        wxString header = _L("Some presets are modified.") + "\n" +
                          (yes_or_no ?
                               _L("You can keep the modified presets for the new project or discard them") :
...
int Plater::save_project(bool saveAs)
{
    ...
    if (export_3mf(into_path(filename), save_strategy) < 0) {
        MessageDialog(this,
                      _L("Failed to save the project.\nPlease check whether the folder exists online or if other programs have the project "
                         "file open."),
...
void Plater::_calib_pa_pattern(const Calib_Params& params)
{
    ...
    if (!has_junction_deviation(printer_config) &&
        print_config.get_abs_value_at("default_jerk", params.extruder_id) > 0) {          // we have set a jerk value
...
void adjust_settings_for_flowrate_calib(ModelObjectPtrs& objects, bool linear, int pass, InfillPattern pattern)
{
    ...
    for (auto _obj : objects) {
        ...
        _obj->config.set_key_value("center_of_surface_pattern",
                                   new ConfigOptionEnum<CenterOfSurfacePattern>(CenterOfSurfacePattern::Each_Surface));
...
void Plater::calib_max_vol_speed(const Calib_Params& params)
{
    ...
    set_config_values<double, ConfigOptionFloats>(filament_config, "filament_max_volumetric_speed",
                                                  std::max(filament_max_volumetric_speed, 200.0));
...
void Plater::calib_input_shaping_freq(const Calib_Params& params)
{
    ...
    if (!add_model(false, Slic3r::resources_dir() + (params.test_model < 1 ? "/calib/input_shaping/ringing_tower.drc" :
    ...
    const double machine_max_speed        = std::min(printer_config->option<ConfigOptionFloats>("machine_max_speed_x")->get_at(0),
                                                     printer_config->option<ConfigOptionFloats>("machine_max_speed_y")->get_at(0));
    ...
    *current_result = std::move(processor.extract_result());
    ...
    if (p->preview->get_canvas3d()->get_gcode_layers_zs().empty()) {
        MessageDialog(this, _L("The selected file") + ":\n" + filename + "\n" + _L("Does not contain valid G-code."),
                      wxString(GCODEVIEWER_APP_NAME) + " - " + _L("An Error has occurred while loading the G-code file."),
...
void Plater::export_gcode(bool prefer_removable)
{
    ...
    try {
        ...
        if (state & priv::UPDATE_BACKGROUND_PROCESS_INVALID)
            return;
        default_output_file = this->p->background_process.output_filepath_for_project(into_path(this->p->get_project_filename(".3mf")));
...
Preset* get_printer_preset(const MachineObject* obj)
{
    ...
    for (auto printer_it = preset_bundle->printers.begin(); printer_it != preset_bundle->printers.end(); printer_it++) {
        ...
        bool nozzle_diameter_matches_or_unknown = printer_nozzle_vals &&
...
void Plater::send_gcode_legacy(int plate_idx, Export3mfProgressFn proFn)
{
...

    {
        ...
        if (host_type == htElegooLink) {
            ...
            pDlg = std::make_unique<CrealityPrintHostSendDialog>(default_output_file, upload_job.printhost->get_post_upload_actions(),
            ...
            PlateData* selected_plate_data = (resolved_plate_idx >= 0 && resolved_plate_idx < static_cast<int>(plate_data_list.size())) ?
                                                 plate_data_list[resolved_plate_idx] :
                                                 nullptr;
            ...
            pDlg = std::make_unique<FlashforgePrintHostSendDialog>(default_output_file, upload_job.printhost->get_post_upload_actions(),
                                                                   groups, storage_paths, storage_names,
                                                                   config->get_bool("open_device_tab_post_upload"), flashforge_host,
                                                                   supports_material_station, std::move(slots), project_filaments);
        } else {
...
void Plater::on_config_change(const DynamicPrintConfig& config)
{
    ...
    for (auto opt_key : diff_keys) {
        ...
        else if (opt_key == "support_interface_filament" || opt_key == "support_filament" || opt_key == "outer_wall_filament_id" ||
                 opt_key == "inner_wall_filament_id" || opt_key == "sparse_infill_filament_id" || opt_key == "internal_solid_filament_id" ||
...
GLCanvas3D* Plater::get_view3D_canvas3D() { return p ? p->view3D->get_canvas3d() : nullptr; }
...
void Plater::validate_current_plate(bool& model_fits, bool& validate_error)
{
    ...
    if (p->printer_technology == ptFFF) {
        ...
        BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                                << boost::format(": validate err=%1%, warnings=%2%, model_fits %3%") % err.string % warnings.size() %
                                       model_fits;
...
bool Plater::refresh_missing_plugin_block(bool* block_toggled)
{
    ...
    const auto update = [&](NotificationType type, const std::vector<MissingPlugin>& missing, std::string* shown_sig,
                            const std::string& header, const std::string& resolve_label,
        ...
        if (*shown_sig != sig) {
            ...
            p->notification_manager->push_plugin_missing_notification(type, header, resolve_label, std::move(body),
                                                                      std::move(resolve_action));
            *shown_sig = sig;
        }
    };
    ...
    update(NotificationType::OrcaCloudPluginMissingError, missing_cloud, &p->m_cloud_missing_shown_sig,
           _u8L("OrcaCloud plugins required by the current preset are not installed:"), _u8L("Install Plugins"),
           [this, missing_cloud_refs](wxEvtHandler*) {
               install_missing_cloud_plugins(missing_cloud_refs);
               return false;
           });
    ...
    update(NotificationType::OrcaLocalPluginMissingError, missing_local, &p->m_local_missing_shown_sig,
           _u8L("Local plugins required by the current preset are missing:"), _u8L("Find on OrcaCloud"),
           [missing_local_refs](wxEvtHandler*) {
               open_missing_plugins_on_cloud(missing_local_refs);
               return false;
           });
    ...
    update(NotificationType::OrcaPluginInactiveError, inactive, &p->m_inactive_shown_sig,
           _u8L("Plugins required by the current preset are not activated:"), _u8L("Activate Now"), [this, inactive_refs](wxEvtHandler*) {
               enable_inactive_plugins(inactive_refs);
               return false;
           });
    update(NotificationType::OrcaPluginCapabilityUnavailableError, broken, &p->m_broken_shown_sig,
           _u8L("The installed plugin does not provide the required capability — it may be outdated:"), _u8L("Find on OrcaCloud"),
           [broken_refs](wxEvtHandler*) {
               open_missing_plugins_on_cloud(broken_refs);
               return false;
           });
...
void Plater::revalidate_current_plate_if_plugins_missing()
{
    ...
    if (p->m_local_missing_shown_sig.empty() && p->m_cloud_missing_shown_sig.empty() && p->m_inactive_shown_sig.empty() &&
        p->m_broken_shown_sig.empty())
        return;
...
void Plater::install_missing_cloud_plugins(const std::vector<std::string>& cloud_refs)
{
    ...
    auto* dialog = new wxProgressDialog(_L("Installing plugins"), from_u8(state->message), 100, parent, wxPD_APP_MODAL | wxPD_CAN_ABORT);
    dialog->Pulse();
...
void Plater::open_filament_map_setting_dialog(wxCommandEvent& evt)
{
    ...
    FilamentMapDialog filament_dlg(this, filament_colors, filament_types, plate_filament_maps, plate_filament_volume_maps,
                                   curr_plate->get_extruders(true), plate_filament_map_mode, this->get_machine_sync_status(), false);
    ...
    if (err.type == StringExceptionType::STRING_EXCEPT_FILAMENT_NOT_MATCH_BED_TYPE) {
        try {
            ...
            if (extruder_id < preset_bundle->filament_presets.size()) {
                ...
                err.string = format(_L("Plate %d: %s is not suggested for use printing filament %s (%s). If you still want to do this "
                                       "print job, please set this filament\'s bed temperature to a number that is not zero."),
...
void Plater::update_filament_volume_map(int extruder_id, int volume_type)
{
    ...
    int selected_volume_type = volume_type == static_cast<int>(NozzleVolumeType::nvtHybrid) ?
    ...
    for (int idx = 0; idx < partplate_list.get_plate_count(); ++idx) {
        ...
        if (filament_map.empty() || filament_volume_map.empty())
            continue;
