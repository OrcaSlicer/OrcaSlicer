...
void ConfigManipulation::toggle_print_fff_options(DynamicPrintConfig *config, const bool is_global_config)
{
    ...
    for (auto el : { "sparse_infill_pattern", "infill_combination", "fill_multiline","infill_direction",
        "minimum_sparse_infill_area", "sparse_infill_filament", "infill_anchor", "infill_anchor_max","infill_shift_step","sparse_infill_rotate_template","symmetric_infill_y_axis","gyroid_optimized"})
    ...
    if (have_infill)
        toggle_field("gyroid_optimized", pattern == ipGyroid);
