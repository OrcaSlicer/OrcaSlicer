...
void ConfigManipulation::update_print_fff_config(DynamicPrintConfig* config, const bool is_global_config, const bool is_plate_config)
{
    ...
    // Dual infill needs Arachne for the same reason fuzzy skin's extrusion modes do: the zone
    // shell is generated by WallToolPaths inside process_arachne(), and process_classic() has no
    // zone code at all. Under the classic generator the zone still gets its floor, ceiling and
    // yolk -- so it looks like it worked -- but no shell wall, which is the part that contains
    // the injected melt laterally. Follow the fuzzy-skin precedent below: offer the fix rather
    // than silently producing a part with nothing holding the injection in.
    bool have_arachne = config->opt_enum<PerimeterGeneratorType>("wall_generator") == PerimeterGeneratorType::Arachne;
    if (config->opt_bool("dual_infill_enabled") && !have_arachne) {
        wxString msg_text = _(L("Dual infill zones require the Arachne Wall Generator. Without it the "
                                "zone is generated with no shell wall, and injected material is not "
                                "contained."));
        msg_text += "\n\n" + _(L("Change these settings automatically?\n"
                                  "Yes - Enable Arachne Wall Generator\n"
                                  "No  - Disable Dual infill zones"));
        MessageDialog dialog(m_msg_dlg_parent, msg_text, "", wxICON_WARNING | wxYES | wxNO);
        DynamicPrintConfig new_conf = *config;
        is_msg_dlg_already_exist = true;
        auto answer = dialog.ShowModal();
        if (answer == wxID_YES)
            ...
            new_conf.set_key_value("dual_infill_enabled", new ConfigOptionBool(false));
        apply(config, &new_conf);
        is_msg_dlg_already_exist = false;
...
void ConfigManipulation::toggle_print_fff_options(DynamicPrintConfig *config, int variant_index, const bool is_global_config)
{
    ...
    for (auto el : { "sparse_infill_pattern", "infill_combination", "fill_multiline","infill_direction",
        "minimum_sparse_infill_area", "filter_narrow_sparse_infill", "sparse_infill_filament_id", "infill_anchor", "infill_anchor_max","infill_shift_step","sparse_infill_rotate_template","symmetric_infill_y_axis"})
...
//    toggle_line("support_interface_speed", have_support_material && have_support_interface);
    ...
    // Upstream retired its half of this block with the per-role filament refactor; these two are
    // fork options with the old 1-based semantics, and picking a per-feature filament is still
    // meaningless on a single-extruder multi-material machine.
    for (auto el : {"dual_infill_outer_filament", "magma_injection_filament"})
