...
PluginDockPanel::PluginDockPanel(wxWindow*          parent,
    ...
    : PluginWebPanel(parent, plugin_web::orca_bridge_script())
    , m_html(html)
...
void PluginDockPanel::request_close()
{
    ...
    // which must return before the web view is destroyed. Every other close path destroys the panel
    // at once, so it can be gone by the time this runs.
    wxWeakRef<PluginDockPanel> self(this);
    CallAfter([self]() {
        if (self)
            self->remove_pane();
    });
