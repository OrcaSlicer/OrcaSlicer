...
static PrinterTechnology factories_printer_technology()
{ return wxGetApp().preset_bundle->printers.get_selected_preset().printer_technology(); }

static int factories_filaments_count() { return wxGetApp().filaments_cnt(); }
    ...
    {{L("Quality"),
     ...
     {L("Support"),
      {{"brim_type", "", 1},
       ...
       {"brim_object_gap", "", 3},
       {"brim_flow_ratio", "", 4},
       {"brim_use_efc_outline", "", 5},
       {"enable_support", "", 6},
       {"support_type", "", 7},
       {"support_threshold_angle", "", 8},
       {"support_threshold_overlap", "", 9},
       {"support_on_build_plate_only", "", 10},
       {"support_filament", "", 11},
       {"support_interface_filament", "", 12},
       {"support_expansion", "", 13},
       {"support_style", "", 14},
       {"tree_support_brim_width", "", 15},
       {"tree_support_branch_angle", "", 16},
       {"tree_support_branch_angle_organic", "", 17},
       {"tree_support_wall_count", "", 18},
       {"tree_support_branch_diameter_angle", "", 19}, // tree support
       {"support_bottom_z_distance", "", 20},
       {"support_top_z_distance", "", 21},
       {"support_base_pattern", "", 22},
       {"support_base_pattern_spacing", "", 23},
       {"support_interface_top_layers", "", 24},
       {"support_interface_bottom_layers", "", 25},
       {"support_interface_spacing", "", 26},
       {"support_bottom_interface_spacing", "", 27},
       {"support_object_xy_distance", "", 28},
       {"bridge_no_support", "", 29},
       {"max_bridge_length", "", 30},
       {"support_critical_regions_only", "", 31},
       {"support_remove_small_overhang", "", 32},
...
{
    if (factories_printer_technology() == ptSLA) {
...
std::vector<SimpleSettingData> SettingsFactory::get_visible_options(const std::string& category, const bool is_part)
{
    /*t_config_option_keys options = {
        ...
        //Shell
        "wall_loops", "top_shell_layers", "bottom_shell_layers", "top_shell_thickness", "bottom_shell_thickness", "top_surface_expansion",
    "top_surface_expansion_margin", "top_surface_expansion_direction",
        //Infill
...
static void get_full_settings_hierarchy(FullSettingsHierarchy& settings_menu, const bool is_part)
{
    ...
    const int filaments_cnt = factories_filaments_count();
...
static void create_freq_settings_popupmenu(wxMenu* menu, const bool is_object_settings, wxDataViewItem item)
{
    ...
    const SettingsFactory::Bundle& bundle = factories_printer_technology() == ptFFF ? FREQ_SETTINGS_BUNDLE_FFF : FREQ_SETTINGS_BUNDLE_SLA;
...
#ifdef __WINDOWS__
    append_menu_item(
        menu, wxID_ANY, _L("Delete") + "\t" + _L_CONTEXT("Del", "Keyboard Shortcut"), _L("Delete the selected object"),
        [](wxCommandEvent&) { plater()->remove_selected(); }, "menu_delete", nullptr, []() { return plater()->can_delete(); }, m_parent);
#else
    append_menu_item(
        menu, wxID_ANY, _L("Delete") + "\t" + _L_CONTEXT("Backspace", "Keyboard Shortcut"), _L("Delete the selected object"),
...
wxMenuItem* MenuFactory::append_menu_item_change_type(wxMenu* menu)
{
    ...
    std::vector<TypeInfo> types = {{ModelVolumeType::MODEL_PART, _L("Part")},
    ...
    for (const auto& info : types) {
        wxMenuItem* item = append_menu_check_item(
            type_menu, wxID_ANY, info.label, "", [type = info.type](wxCommandEvent&) { obj_list()->set_volume_type(type); }, type_menu);

        // Update checkmark dynamically when menu is shown - check all selected volumes
        m_parent->Bind(
            wxEVT_UPDATE_UI,
            [type = info.type](wxUpdateUIEvent& evt) {
                ...
                for (auto item : sels) {
                    if (model->GetItemType(item) == itVolume) {
                        ...
                        if (vol != nullptr && (vol->is_svg() || vol->is_text()) &&
                            (type == ModelVolumeType::SUPPORT_BLOCKER || type == ModelVolumeType::SUPPORT_ENFORCER)) {
                            evt.Enable(false);
                            break;
                        }
                    }
...
{
    // BBS: to be checked
    wxMenuItem* menu_item_printable = append_menu_check_item(
        menu, wxID_ANY, _L("Printable") + "\t" + "V", "", [](wxCommandEvent&) { obj_list()->toggle_printable_state(); }, menu);
...
wxMenuItem* MenuFactory::append_menu_item_auto_drop(wxMenu* menu)
{
    ...
    wxMenuItem* menu_item_auto_drop =
        append_menu_check_item(menu, wxID_ANY, menu_text, menu_tooltip, [](wxCommandEvent&) { obj_list()->toggle_auto_drop(); }, menu);
...
{
    append_menu_item(
        menu, wxID_ANY, _L("Assemble"), _L("Assemble the selected objects into an object with multiple parts"),
...
void MenuFactory::append_menu_item_merge_to_single_object(wxMenu* menu)
{
    menu->AppendSeparator();
    append_menu_item(
        menu, wxID_ANY, _L("Assemble"), _L("Assemble the selected objects into an object with single part"),
...
void MenuFactory::append_menu_item_change_filament(wxMenu* menu)
{
    ...
    int filaments_cnt = factories_filaments_count();
    if (filaments_cnt <= 1)
        return;
