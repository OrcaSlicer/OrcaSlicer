        ...
        )
    {
		...
		// environment correctly targets and removes the titlebar for this screen.
            ...
            if (Slic3r::GUI::is_running_on_wayland()) {
                ...
                auto apply_no_titlebar = [](GtkWidget *w) {
                ...
                if (gtk_widget_get_realized(widget)) {
                    // wxSplashScreen's own base constructor already called Show(true),
                    // which realizes (and on Wayland, maps) the GtkWindow before our
                    // derived constructor body runs. gtk_window_set_titlebar() only
                    // works pre-realize, so retroactively force an unrealize/realize
                    // cycle to apply it. Hide first to avoid a visible flash of the
                    // default decorated window.
                    bool was_visible = gtk_widget_get_visible(widget);
                    if (was_visible)
                        gtk_widget_hide(widget);

                    gtk_widget_unrealize(widget);
                    apply_no_titlebar(widget);
