    ...
    , m_mapping_tutorial_popup(AmsTutorialPopup(this))
{
   ...
   m_mode_send->Bind(wxEVT_LEFT_DOWN, [this](auto& e) {
    ...
    m_timelapse_folder_btn = new ScalableButton(m_options_other, wxID_ANY, "folder-closed", wxEmptyString, wxDefaultSize, wxDefaultPosition,
    ...
    m_timelapse_folder_btn->Bind(wxEVT_BUTTON, [this](wxCommandEvent&) { show_timelapse_folder_popup(); });
    ...
    auto option_pa_value = new PrintOption(m_options_other, _L("Shared PA Profile"),
                                           _L("Nozzles and filaments of the same type share the same PA profile."), ops_no_auto,
                                           "pa_value");
    ...
    for (auto print_opt : m_checkbox_list_order) {
        print_opt->Bind(EVT_SWITCH_PRINT_OPTION, [this, print_opt](auto& e) {
            ...
            if (print_opt == m_checkbox_list["flow_cali"])
                on_flow_cali_option_changed();
            else if (print_opt == m_checkbox_list["pa_value"])
                on_pa_value_option_changed();
        });
    }
...
void SelectMachineDialog::sync_ams_mapping_result(std::vector<FilamentInfo>& result)
{
    ...
    auto relayout_nozzle_cards = [this]() {
        ...
        if (!obj_->GetFilaSwitch()->IsInstalled() && !(ns && ns->GetNozzleRack()->IsSupported()))
            return;
        if (m_sizer_ams_mapping_left) {
            m_sizer_ams_mapping_left->Layout();
            m_filament_panel_left_sizer->Layout();
            m_filament_left_panel->Layout();
        }
        if (m_sizer_ams_mapping_right) {
            m_sizer_ams_mapping_right->Layout();
            m_filament_panel_right_sizer->Layout();
            m_filament_right_panel->Layout();
        }
        if (m_sizer_ams_mapping) {
...
bool SelectMachineDialog::can_hybrid_mapping(MachineObject* obj_) const { return obj_ && obj_->GetFilaSwitch()->IsInstalled(); }
...
bool SelectMachineDialog::use_dynamic_nozzle_map() const
{
    if (m_print_type == FROM_NORMAL) {
        ...
        if (m_required_data_plate_data_list.size() > (size_t) m_print_plate_idx) {
            auto dynamic_nozzle_map = m_required_data_plate_data_list[m_print_plate_idx]->config.option<ConfigOptionBool>(
...
bool SelectMachineDialog::slicing_with_fila_switch() const
{
    ...
    if (m_print_type == FROM_NORMAL) {
        ...
        if (m_required_data_plate_data_list.size() > (size_t) m_print_plate_idx) {
            auto has_filament_switcher = m_required_data_plate_data_list[m_print_plate_idx]->config.option<ConfigOptionBool>(
...
bool SelectMachineDialog::CheckErrorDynamicSwitchNozzle(MachineObject* obj_)
{
    ...
    if (obj_->is_support_check_track_switch_match_slice_printer && slicing_with_fila_switch() != obj_->GetFilaSwitch()->IsInstalled()) {
        show_status(PrintDialogStatus::PrintStatusFilaSwitcherSlicingNotMatch,
                    {_L("The Filament Track Switch installed on the printer does not match the slicing file. Please re-slice to avoid "
    ...
    if (!obj_->GetFilaSwitch()->IsInstalled()) {
        show_status(PrintDialogStatus::PrintStatusFilaSwitcherError,
                    {_L("This print requires a Filament Track Switch. Please install it first.")});
        return false;
    }

    if (!obj_->GetFilaSwitch()->IsReady()) {
        show_status(PrintDialogStatus::PrintStatusFilaSwitcherError,
                    {_L("The Filament Track Switch has not been setup. Please setup it first.")});
        return false;
    }
...
void SelectMachineDialog::update_pa_value_option(MachineObject* obj)
{
    ...
    const bool show_pa = obj && obj->is_support_pa_mode && m_checkbox_list["flow_cali"]->IsShown() &&
...
void SelectMachineDialog::on_pa_value_option_changed()
{
    ...
    DeviceManager* dev  = wxGetApp().getDeviceManager();
    MachineObject* obj_ = dev ? dev->get_selected_machine() : nullptr;
    if (!obj_ || !(obj_->GetNozzleSystem() && obj_->GetNozzleSystem()->GetNozzleRack()->IsSupported()))
        return;
    if (use_dynamic_nozzle_map())
        return;
    clear_nozzle_mapping();
...
bool SelectMachineDialog::CheckErrorSyncNozzleMappingResultV1(MachineObject* obj_)
{
    ...
    if (!obj_nozzle_mapping_ptr->HasResult()) {
        if (time(nullptr) - s_nozzle_mapping_last_request_time > 10) { // avoid too many requests
            ...
            if (rtn == 0) {
                ...
                const auto& err_msg = wxString::Format(
                    _L("Failed to send nozzle auto-mapping request to printer { code: %d }. "
                       "Please try to refresh the printer information. "
                       "If it still does not recover, you can try to rebind the printer and check the network connection."),
                    rtn);
    ...
    if (obj_nozzle_mapping_ptr->GetResultStr() == "failed") {
        const auto& err_msg = wxString::Format(
            _L("Failed to receive nozzle auto-mapping table from printer { msg: %s }. Please refresh the printer information."),
    ...
    if (obj_nozzle_mapping_ptr->GetResultStr() == "fail") {
        const wxString& err_msg = wxString::Format(
            _L("The printer failed to build the nozzle auto-mapping table { code: %d }. Please refresh nozzle information."),
    ...
    if (!mapping_map.empty()) {
        ...
        if ((flush_waste_base != -1) && (flush_waste_current != -1) && flush_waste_current > flush_waste_base) {
            const wxString& warning_msg = wxString::Format(_L("The current nozzle mapping may produce an extra %0.2f g of waste."),
                                                           flush_waste_current - flush_waste_base);
    ...
    const wxString& err_msg = wxString::Format(
        _L("Failed to receive nozzle auto-mapping table from printer { msg: %s }. Please refresh the printer information."), "empty table");
    show_status(PrintDialogStatus::PrintStatusRackNozzleMappingError, {err_msg});
    return true;
}
    ...
    if (!obj_nozzle_mapping_ptr->HasResult()) {
        if (time(nullptr) - s_nozzle_mapping_last_request_time > 10) { // avoid too many requests
            ...
            int rtn            = obj_nozzle_mapping_ptr->CtrlGetAutoNozzleMappingV0(m_plater, m_ams_mapping_result,
                                                                                    m_checkbox_list["flow_cali"]->getValueInt(), pa_value);
            if (rtn == 0) {
                s_nozzle_mapping_last_request_time = time(nullptr);
            } else {
                const auto& err_msg = wxString::Format(
...
bool SelectMachineDialog::check_sdcard_for_timelpase(MachineObject* obj)
{
    ...
    if (m_checkbox_list["timelapse"]->IsShown() && m_checkbox_list["timelapse"]->getValue() == "on") {
        if (obj->GetStorage()->get_sdcard_state() == DevStorage::SdcardState::NO_SDCARD && !obj->m_has_timelapse_kit &&
            !obj->is_support_internal_timelapse) {
...
{
    if (!obj_ || m_print_type != PrintFromType::FROM_NORMAL)
        return true;
...
bool SelectMachineDialog::CheckWarningSmartNozzleBlobAuto(MachineObject* obj_)
{
    ...
    if (!opt_nozzle_diameters || opt_nozzle_diameters->values.empty())
        return true;

    for (const auto& fila : m_filaments) {
        if (fila.filament_id.empty())
            continue;
...
wxString SelectMachineDialog::FormatTime(float totalSeconds)
{
    ...
    if (hours > 0)
        return wxString::Format("%dm%ds", hours * 60 + minutes, seconds);
    if (minutes > 0)
        return wxString::Format("%dm%ds", minutes, seconds);
...
std::optional<float> SelectMachineDialog::get_filament_change_gap_time(MachineObject* obj_) const
{
    ...
    try {
        auto r = MultiNozzleUtils::simulate_filament_change_time(logic_filaments, nozzle_list, fila_change_seq, nozzle_change_seq,
                                                                 group_of_filaments, params, ams_preload_enabled,
                                                                 /*calc_sliced_time=*/true);
...
std::map<int, DevFilaSwitch::SwitchPos> SelectMachineDialog::get_filament_suggest_pos(MachineObject* obj_) const
{
    ...
    if (pos2group.size() == 1) {
        DevFilaSwitch::SwitchPos target = (in_a_fila_set.size() >= in_b_fila_set.size()) ? DevFilaSwitch::POS_IN_A :
                                                                                           DevFilaSwitch::POS_IN_B;
        for (const auto& item : m_ams_mapping_result)
...
{
    if (m_print_type != PrintFromType::FROM_NORMAL || !m_saveTimeText)
        return;
...
{
    if (!m_best_pos_dialog)
        return; // Orca: only relevant while the popup is open
    DeviceManager* dev  = wxGetApp().getDeviceManager();
    MachineObject* obj_ = dev ? dev->get_selected_machine() : nullptr;
    if (!obj_)
        return;
...
void SelectMachineDialog::show_status(PrintDialogStatus status, std::vector<wxString> params, wxString wiki_url)
{
    ...
    if (status == PrintDialogStatus::PrintStatusNozzleDiameterMismatch) {
        ...
        m_pre_print_checker.add_with_checkbox(status, msg, _L("I have checked the installed nozzle and want to print anyway."),
                                              !m_nozzle_diameter_ack_msg.empty() &&
                                                  m_nozzle_diameter_ack_msg == m_nozzle_diameter_mismatch_msg,
                                              [this](bool checked) {
...
void SelectMachineDialog::on_ok_btn(wxCommandEvent& event)
{
    ...
    for (auto i = 0; i < m_ams_mapping_result.size(); i++) {
        ...
        for (const auto& [pos_id, nozzle] : get_mapped_nozzles(m_ams_mapping_result[i].id)) {
            if (!nozzle.IsEmpty()) {
                mapped_nozzles.push_back(nozzle);
            }
        }
    ...
    if (has_unknown_filament) {
        ...
        confirm_text.push_back(
            ConfirmBeforeSendInfo(_L("There are some unknown filaments in the AMS mappings. Please check whether they are the required "
                                     "filaments. If they are okay, click \"Confirm\" to start printing.")));
    }
    ...
    if (has_slice_warnings) {
        ...
        if (obj_->is_support_internal_timelapse && m_checkbox_list["timelapse"]->IsShown() &&
            m_checkbox_list["timelapse"]->getValue() == "on" && !m_timelapse_storage.empty()) {
    ...
    auto make_item = [&](const wxString& label, const std::string& val, bool enabled) {
        ...
        if (enabled) {
            auto on_select = [this, val](wxMouseEvent&) {
                ...
                update_timelapse_folder_btn_icon();
                if (m_timelapse_storage_popup)
                    m_timelapse_storage_popup->Dismiss();
                DeviceManager* dev = wxGetApp().getDeviceManager();
                MachineObject* obj = dev ? dev->get_selected_machine() : nullptr;
                if (obj)
                    check_timelapse_storage_warning(obj);
            };
...
{
    if (!obj || !obj->is_support_internal_timelapse)
        return;
    ...
    if (obj->is_timelapse_storage_low(m_timelapse_storage)) {
        ...
        wxString msg          = wxString::Format(_L("%s space less than 20MB. Timelapse may not save properly. You can turn it off or"),
                                                 storage_name);
        ...
        m_pre_print_checker.add_with_link(PrintDialogStatus::PrintStatusTimelapseStorageLow, msg, _L("Clean up files"),
...
void SelectMachineDialog::show_timelapse_storage_dialog(MachineObject* obj)
{
    ...
    if (is_internal)
        ...
        body_text = _L(
            "Insufficient external storage for time-lapse photography. Connect to computer to delete files, or use a larger memory card.");

    wxDialog dlg(this, wxID_ANY, _L("Storage Space Not Enough"), wxDefaultPosition, wxDefaultSize, wxDEFAULT_DIALOG_STYLE);
    ...
    auto* warn_bmp  = new wxStaticBitmap(&dlg, wxID_ANY, create_scaled_bitmap("obj_warning", &dlg, 16), wxDefaultPosition,
                                         wxSize(FromDIP(16), FromDIP(16)));
...
void SelectMachineDialog::on_send_print()
{
    ...
    m_print_job->set_print_config(MachineBedTypeString[0], (m_checkbox_list["bed_leveling"]->getValue() == "on"),
                                  ...
                                  m_checkbox_list["nozzle_offset_cali"]->getValueInt(), pa_manual_mode);
        ...
        for (int used_filament_idx : used_filament_idxs) {
            ...
            if (volume_type == NozzleVolumeType::nvtHybrid) {
                ...
                if (nozzle_group_res) {
                    ...
                    BOOST_LOG_TRIVIAL(warning) << __FUNCTION__ << ": no nozzle group result, nozzle check skipped for extruder "
                                               << logic_extruder_idx;
                }
...
bool SelectMachineDialog::CheckErrorRackStatus(MachineObject* obj_)
{
    ...
    if (rack->GetReadingCount() > 0) {
        const wxString& msg = wxString::Format(_L("Refreshing information of hotends(%d/%d)."), rack->GetReadingIdx(),
                                               rack->GetReadingCount());
...
void SelectMachineDialog::CheckWarningRackStatus(MachineObject* obj_)
{
    ...
    for (const auto& need_nozzle : need_nozzle_map) {
        ...
        const auto& installed_nozzles = nozzle_sys->CollectNozzles(MAIN_EXTRUDER_ID, nozzle_info.nozzle_flow_type,
                                                                   nozzle_info.nozzle_diameter);
        ...
        if (need_nozzle.second > installed_reliable_count && nozzle_sys->HasUnreliableNozzles()) {
            ...
            show_status(PrintDialogStatus::PrintStatusHasUnreliableNozzleWarning,
                        {_L("The reported hotend information may be unreliable.") + " " +
                         _L("Please refresh the nozzle information and try again.")});
        }
    }
}
...
bool SelectMachineDialog::CheckErrorExtruderNozzleWithSlicing(MachineObject* obj_)
{
    ...
    if (m_print_type == FROM_NORMAL) {
        ...
        for (auto slicing_ext_nozzle : slicing_ext_nozzles) {
            ...
            if (slicing_ext_idx == MAIN_EXTRUDER_ID && nozzle_sys->GetNozzleRack()->IsSupported()) {
                if (nozzle_sys->CollectNozzles(MAIN_EXTRUDER_ID, slicing_ext.nozzle_flow_type, slicing_ext.nozzle_diameter).empty()) {
                    ...
                    if (nozzle_sys->GetNozzleRack()->GetCaliStatus() != DevNozzleRack::Rack_CALI_OK)
                        ...
                        msg += _L(
                            "Please install a matching nozzle in the hotend rack, or set the corresponding printer preset while slicing.");
            ...
            {
                if (installed_ext_nozzle.GetNozzleType() == NozzleType::ntUndefine || installed_ext_nozzle.GetNozzleDiameter() <= 0.0f) {
                    show_status(PrintDialogStatus::PrintStatusNozzleDataInvalid);
                    return false;
                }

                if (obj_->is_nozzle_flow_type_supported() && installed_ext_nozzle.GetNozzleFlowType() == NozzleFlowType::NONE_FLOWTYPE) {
                    show_status(PrintDialogStatus::PrintStatusNozzleDataInvalid);
                    return false;
                }
            }
            ...
            // check nozzle flow type
            {
                if (obj_->is_nozzle_flow_type_supported() && slicing_ext.nozzle_flow_type != installed_ext_nozzle.GetNozzleFlowType()) {
                    ...
                    wxString error_message =
                        wxString::Format(_L("The nozzle flow setting of %s(%s) doesn't match with the slicing file(%s). "
                    ...
                    params.emplace_back(_L("Tips: If you changed your nozzle of your printer lately, please go to 'Device -> Printer "
                                           "parts' to change your nozzle setting."));
            ...
            // check nozzle diameter
            {
                if (slicing_ext.nozzle_diameter != installed_ext_nozzle.GetNozzleDiameter()) {
                    ...
                    msg_params.emplace_back(_L("Tips: If you changed your nozzle of your printer lately, please go to 'Device -> Printer "
                                               "parts' to change your nozzle setting."));
...
void SelectMachineDialog::update_show_status(MachineObject* obj_)
{
    ...
    if (obj_->is_lan_mode_printer()) {
        if (obj_->GetStorage()->get_sdcard_state() == DevStorage::SdcardState::NO_SDCARD && !obj_->can_use_emmc_print()) {
    ...
    if (m_print_type == PrintFromType::FROM_NORMAL) {
        ...
        for (const auto& extder : obj_->GetExtderSystem()->GetExtruders()) {
            ...
            if (_is_rack_managed_nozzle(obj_, extder.GetNozzleId())) {
                continue;
            }
    ...
    if (DevPrinterConfigUtil::support_print_check_firmware_for_tpu_left(obj_->printer_type)) {
        ...
        if (has_tpu_left && !obj_->m_firmware_support_print_tpu_left.value_or(false)) {
            show_status(
                PrintDialogStatus::PrintStatusFirmwareNotSupportTpuAtLeft,
    ...
    for (auto i = 0; i < m_ams_mapping_result.size(); i++) {
        ...
        for (size_t n = 0; n < iterations; ++n) {
            ...
            if (const auto& warning_items = result.get_items_by_action("warning"); !warning_items.empty()) {
                for (const auto& item : warning_items) {
                    show_status(PrintDialogStatus::PrintStatusHasFilamentInBlackListWarning, {item.info_msg},
                                item.wiki_url); /** warning check **/
                }
            }
            ...
            if (n < mapped_nozzles.size()) {
                ...
                if (_is_rack_managed_nozzle(obj_, nozzle.GetExtruderId()) && nozzle.GetNozzleType() != NozzleType::ntUndefine) {
                    ...
                    if (abs(filament_hrc) > abs(nozzle_hrc)) {
                        ...
                        show_status(
                            PrintDialogStatus::PrintStatusFilamentWarningNozzleHRC,
                            {wxString::Format(
                                _L("The hardness of current material (%s) exceeds the hardness of %s(%s). It may cause nozzle wear, "
    ...
    if (!CheckWarningFilamentCrossExtruder(obj_)) {
        wxString warning_msg = _L("Some filaments may switch between extruders during printing. Manual K-value calibration cannot be "
                                  "applied throughout the entire print, which "
    ...
    if (!CheckWarningSmartNozzleBlobAuto(obj_)) {
        ...
        m_pre_print_checker.add_with_link(PrintDialogStatus::PrintStatusSmartNozzleBlobNeedAuto,
                                          _L("There is stringing-prone filament in this file. For best print quality, we recommend "
                                             "switching nozzle clumping detection to Auto mode."),
                                          _L("Switch"), [] {
                                              DeviceManager* dev = wxGetApp().getDeviceManager();
            ...
            for (const auto& fila : m_ams_mapping_result) {
                if (std::find(warning_tpu_filaments.begin(), warning_tpu_filaments.end(), fila.filament_id) != warning_tpu_filaments.end()) {
                    show_status(PrintDialogStatus::PrintStatusTPUUnsuggestCali,
                                {_L("If 'Dynamic Flow Calibration' is set to Auto/On, the system will use the manual calibration value or "
                                    "the default value and skip the flow calibration process. You can perform a manual flow calibration "
                                    "for TPU filament on the 'Calibration' page.")});
                    break;
                }
            }
        }
