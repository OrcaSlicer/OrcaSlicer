...
void GLGizmoMmuSegmentation::on_render_input_window(float x, float y, float bottom_limit)
{
    ...
    for (int extruder_idx = 0; extruder_idx < n_extruder_colors; extruder_idx++) {
        ...
        if (draw_color_button(
            filament_index_from_zero_based(extruder_idx), // idx
        ...
        if (extruder_idx < 16 && ImGui::IsItemHovered()) m_imgui->tooltip(_L("Shortcut Key ") + std::to_string(filament_index_from_zero_based(extruder_idx)), max_tooltip_width);
    }
...
void GLGizmoMmuSegmentation::render_filament_remap_ui(float window_width, float max_tooltip_width, float scale)
{
    ...
    for (size_t src : m_used_filaments) {
        ...
        bool src_clicked = draw_color_button(
            filament_index_from_zero_based(static_cast<int>(src)), // idx
        ...
        if (ImGui::IsItemHovered() && src != m_extruder_remap[src]) // show tooltip if it has mapping info
            m_imgui->tooltip(std::to_string(filament_index_from_zero_based(static_cast<int>(src))) + " >> " +
                                 std::to_string(filament_index_from_zero_based(static_cast<int>(m_extruder_remap[src]))),
                             max_tooltip_width);
        ...
        if (ImGui::BeginPopup(pop_id.c_str())) {
            ...
            for (int dst = 0; dst < (int)n_extr; ++dst) {
                ...
                bool dst_clicked = draw_color_button(
                    filament_index_from_zero_based(dst), // idx
