...
std::string GLGizmoCut3D::get_tooltip() const
{
    ...
    if (m_hover_id == Z || (m_dragging && m_hover_id == CutPlane)) {
        ...
        std::string unit_str = " " + (m_imperial_units ? _CTX_utf8(L_CONTEXT("in", "inches")) : _u8L("mm"));
...
bool GLGizmoCut3D::render_double_input(const std::string& label, double& value_in)
{
    ...
    ImGui::SameLine();
    m_imgui->text(m_imperial_units ? _CTX(L_CONTEXT("in", "inches")) : _L("mm"));
...
bool GLGizmoCut3D::render_slider_two_input(const std::string& label, float& value_in, float& tolerance_in, float min_val/* = -0.1f*/, float max_tolerance/* = -0.1f*/)
{
    ...
    std::string format = value_in < 0.f ? " " : m_imperial_units ? "%.4f " + _CTX_utf8(L_CONTEXT("in", "inches")) : "%.2f " + _u8L("mm");
...
{
    double   koef     = m_imperial_units ? GizmoObjectManipulation::mm_to_in : 1.0;
    wxString unit_str = m_imperial_units ? _CTX(L_CONTEXT("in", "inches")) : _L("mm");
