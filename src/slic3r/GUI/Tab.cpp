...
// Orca: we use a different logic here. If SEMM is enabled, we set extruder count to 1.
    ...
    for (auto extruder_idx = m_extruders_count_old; extruder_idx < m_extruders_count; ++extruder_idx) {
        const wxString page_name = (m_extruders_count > 1) ? wxString("Extruder ") + format_filament_index_for_display(static_cast<int>(extruder_idx) + 1) : wxString("Extruder");
    ...
    for (size_t extruder_idx = 0; extruder_idx < m_extruders_count; ++extruder_idx) {
        wxString page_title = (m_extruders_count > 1) ? wxString("Extruder ") + format_filament_index_for_display(static_cast<int>(extruder_idx) + 1) : wxString("Extruder");
