...
CameraEditDialog::CameraEditDialog(
    wxWindow* parent, const std::string& dev_id, const std::string& url, CameraSourceType source_type, bool enabled)
...
void CameraEditDialog::create_ui()
{
    ...
    m_source_type_combo = new wxComboBox(this, wxID_ANY, wxEmptyString, wxDefaultPosition, wxSize(FromDIP(250), -1), source_types,
                                         wxCB_READONLY);
...
void CameraEditDialog::populate_printer_list()
{
    ...
    for (const auto& pair : local_machines) {
        if (has_existing_override(pair.first))
            continue;
        bool exists = false;
        for (const auto& p : m_printers) {
...
std::string CameraEditDialog::get_url() const { return m_url_input->GetTextCtrl()->GetValue().ToStdString(); }
...
bool CameraEditDialog::get_enabled() const { return m_enabled_checkbox->GetValue(); }

void CameraEditDialog::on_source_type_changed(wxCommandEvent& event) { update_url_field_state(); }
...
CameraManagementDialog::CameraManagementDialog(wxWindow* parent)
    : DPIDialog(parent,
                ...
                _L("Camera Overrides"),
                wxDefaultPosition,
                wxSize(FromDIP(550), FromDIP(400)),
...
CameraManagementDialog::~CameraManagementDialog() { cleanup_list_data(); }
...
void CameraManagementDialog::on_delete(wxCommandEvent& event)
{
    ...
    int result = wxMessageBox(_L("Are you sure you want to delete this camera override?"), _L("Confirm Delete"), wxYES_NO | wxICON_QUESTION,
                              this);
    if (result == wxYES) {
...
std::string CameraManagementDialog::truncate_serial(const std::string& dev_id) { return dev_id; }

void CameraManagementDialog::on_dpi_changed(const wxRect& suggested_rect) { Layout(); }
