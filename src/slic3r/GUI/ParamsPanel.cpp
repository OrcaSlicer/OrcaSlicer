...
namespace {
int mode_to_selection_panel(ConfigOptionMode mode) { return mode == comExpert ? 2 : mode == comAdvanced ? 1 : 0; }
} // namespace
    ...
    : wxPanel(parent, id, pos, size, style, name)
{
    ...
    if (dynamic_cast<Notebook*>(parent)) {
        ...
        m_mode_view->SetSelection(mode_to_selection_panel(isDevMode ? comExpert : wxGetApp().get_saved_mode()));
...
void ParamsPanel::update_mode()
{
    ...
    auto sync_mode_view = [&](ModeSwitchButton* mode_view) {
        ...
        if (app_mode == comDevelop) {
            mode_view->SetSelection(mode_to_selection_panel(comExpert));
            mode_view->SetDevMode(true);
            return;
        }

        mode_view->SetSelection(mode_to_selection_panel(Slic3r::GUI::wxGetApp().get_saved_mode()));
