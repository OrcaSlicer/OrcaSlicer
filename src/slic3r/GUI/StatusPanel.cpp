...
wxBoxSizer* StatusBasePanel::create_extruder_control(wxWindow* parent)
{
    ...
    m_nozzle_btn_panel = new SwitchBoard(panel, _L_CONTEXT("Left", "Nozzle position"), _L_CONTEXT("Right", "Nozzle position"),
                                         wxSize(FromDIP(126), FromDIP(26)));
...
void StatusPanel::update_ams_control_state(std::string ams_id, std::string slot_id)
{
    ...
    if (obj->is_in_printing() && !obj->can_resume()) {
        ...
        if (ams_id.empty() || slot_id.empty()) {
            ...
            for (auto ext : obj->GetExtderSystem()->GetExtruders()) {
                // Orca: with a Filament Track Switch installed a slot is not bound to a single extruder, so skip the "already loaded"
                // identity check.
            ...
            if (!ams_item) {
                load_error_info = _L(
                    "Choose an AMS slot then press \"Load\" or \"Unload\" button to automatically load or unload filament.");
            } else {
        ...
        if (obj->is_in_prepare() || obj->print_status == "SLICING") {
            ...
            if (obj->is_in_prepare()) {
                ...
                if (obj->queue_number <= 0) {
                    ...
                    prepare_text = wxString::Format(_L("In Cloud Slicing Queue, there are %s tasks ahead of you."),
    ...
    if (obj) {
        ...
        if (obj->GetFilaSwitch() && obj->GetFilaSwitch()->IsInstalled()) {
            if (!obj->GetFilaSwitch()->IsReady()) {
                MessageDialog msg_dlg(nullptr, _L("The Filament Track Switch has not been setup. Please setup on printer."), wxEmptyString,
                                      wxICON_WARNING | wxOK);
                msg_dlg.ShowModal();
                return;
            }
...
void StatusPanel::update_filament_loading_panel(MachineObject* obj)
{
    ...
    m_fila_change_abort->Show(
        ams_loading_state && (obj->is_support_fila_change_abort || DevPrinterConfigUtil::support_ams_fila_change_abort(obj->printer_type)));
...
wxBoxSizer* ScoreDialog::get_photo_btn_sizer()
{
    ...
    m_add_photo->Bind(wxEVT_LEFT_DOWN, [this](auto& e) {
        // add photo logic
        wxFileDialog openFileDialog(this, _L("Select Images"), "", "", _L("Image files (*.png;*.jpg;*jpeg)|*.png;*.jpg;*.jpeg"),
...
#undef TEMP_THRESHOLD_VAL
#undef TEMP_THRESHOLD_ALLOW_E_CTRL
#undef PAGE_TITLE_HEIGHT
#undef PAGE_TITLE_TEXT_WIDTH
#undef PAGE_TITLE_LEFT_MARGIN
#undef GROUP_TITLE_LEFT_MARGIN
#undef GROUP_TITLE_LINE_MARGIN
#undef GROUP_TITLE_RIGHT_MARGIN
#undef NORMAL_SPACING
#undef PAGE_SPACING
#undef PAGE_MIN_WIDTH
#undef PROGRESSBAR_HEIGHT
#undef SWITCH_BUTTON_SIZE
#undef TASK_THUMBNAIL_SIZE
#undef TASK_BUTTON_SIZE
#undef TASK_BUTTON_SIZE2
#undef Z_BUTTON_SIZE
#undef MISC_BUTTON_PANEL_SIZE
#undef MISC_BUTTON_1FAN_SIZE
#undef MISC_BUTTON_2FAN_SIZE
#undef MISC_BUTTON_3FAN_SIZE
#undef TEMP_CTRL_MIN_SIZE_ALIGN_ONE_ICON
#undef TEMP_CTRL_MIN_SIZE_ALIGN_TWO_ICON
#undef AXIS_MIN_SIZE
#undef EXTRUDER_IMAGE_SIZE
#undef SWITCHING_STATUS_BTN_SIZE
