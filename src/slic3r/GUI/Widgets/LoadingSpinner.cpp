...
LoadingSpinner::LoadingSpinner(wxWindow* parent, const wxSize& size) : m_timer_interval(20), m_animation_duration(1200)
{
...
void LoadingSpinner::SetUpdateInterval(const int update_interval_ms) { m_timer_interval = update_interval_ms; }

void LoadingSpinner::SetAnimationDuration(const int duration_ms) { m_animation_duration = duration_ms; }
...
{
    const auto start_time_diff = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() - m_timer_start)
                                     .count();
