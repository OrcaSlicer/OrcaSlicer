...
static const wxString NA_STR_CALI = _L("N/A");
...
void CalibrationCaliPage::update_subtask(MachineObject* obj)
{
    ...
    if (obj->is_system_printing() || obj->is_in_calibration()) {
        ...
        if (obj->is_in_prepare() || obj->print_status == "SLICING") {
            ...
            m_printing_panel->update_progress_percent(NA_STR_CALI, wxEmptyString);
            m_printing_panel->update_left_time(NA_STR_CALI);
            m_printing_panel->update_layers_num(true, wxString::Format(_L("Layer: %s"), NA_STR_CALI));
...
void CalibrationCaliPage::reset_printing_values()
{
    ...
    m_printing_panel->update_subtask_name(NA_STR_CALI);
    m_printing_panel->show_profile_info(false);
