...
// BBS: new layout
constexpr int options_group_title_width = 20;
...
void OptionsGroup::activate_line(Line& line)
{
    ...
    if (line.full_width && (line.widget != nullptr || !line.get_extra_widgets().empty())) {
        ...
        if (!line.get_extra_widgets().empty()) {
            ...
            for (auto extra_widget : line.get_extra_widgets()) {
                h_sizer->Add(extra_widget(this->ctrl_parent()), is_first_item ? 1 : 0, wxLEFT, options_group_title_width * wxGetApp().em_unit());
                is_first_item = false;
            }
            return;
        }
    ...
    if (option_set.size() == 1 && label_width == 0 && option_set.front().opt.full_width && option_set.front().opt.sidetext.size() == 0 &&
        ...
        int left_padding = option.opt.multiline ? 0 : options_group_title_width * wxGetApp().em_unit();
        if (m_labels_hidden)
            left_padding = 0;
