...
public:
    ...
    // Publish any staged local changes to the remote. For live backends
    // (WebDAV) uploads/deletes hit the server immediately, so this is a no-op.
    // Git only writes+stages files in upload_file/delete_file and must commit
    // and push them here -- without this call the Git backend never reaches
    // the remote.
    virtual bool flush(std::string& error_out) { (void) error_out; return true; }
