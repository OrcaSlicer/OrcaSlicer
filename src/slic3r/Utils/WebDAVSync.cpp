                              ...
                              SyncError* error_code_out)
{
    ...
    // Some WebDAV servers (nginx's dav module, certain Apache mod_dav configs)
    // do not echo an ETag on the PUT response. Without it the provider would
    // cache an empty etag, the next push would omit If-Match, and optimistic
    // concurrency (conflict detection) would silently stop working. Recover it
    // with a targeted PROPFIND so OCC keeps holding on those servers.
    if (new_etag_out.empty())
        new_etag_out = fetch_remote_etag(remote_path);
    return true;
}

std::string WebDAVSync::fetch_remote_etag(const std::string& remote_path)
{
    std::string url = build_url(remote_path);
    std::string response_body;
    bool        success = false;
    ...
    http.header("Depth", "0")
        ...
        .on_error([&](std::string body, std::string, unsigned status) {
            // 207 Multi-Status is the normal PROPFIND reply but arrives via
            // on_error because the Http wrapper treats non-2xx as errors.
            if (status == 207) {
                response_body = std::move(body);
                success       = true;
            }
    ...
    if (!parse_propfind_response(response_body, remote_path, entries))
        return {};
