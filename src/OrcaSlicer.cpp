...
static int run_cli_or_agent(int argc, char **argv)
{
    if (agent_invocation(argc, argv)) {
        AgentCliResult agent = agent_cli_run(argc, argv);
        if (!agent.forward_to_legacy) {
            if (!agent.stdout_text.empty())
                boost::nowide::cout << agent.stdout_text << std::endl;
            return agent.exit_code;
        }
        std::vector<std::string> storage;
        storage.reserve(agent.legacy_args.size() + 1);
        storage.emplace_back(argv[0] != nullptr ? argv[0] : "orca-slicer");
        storage.insert(storage.end(), agent.legacy_args.begin(), agent.legacy_args.end());
        std::vector<char *> pointers;
        pointers.reserve(storage.size() + 1);
        for (std::string &arg : storage)
            pointers.push_back(arg.data());
        pointers.push_back(nullptr);
        return CLI().run(static_cast<int>(storage.size()), pointers.data());
    }
...
//BBS: register default exception handler
        ...
        // Call the UTF8 main.
        return run_cli_or_agent(argc, argv_ptrs.data());
    }
}
#else /* _MSC_VER */
...
#endif
    return run_cli_or_agent(argc, argv);
}
#endif /* _MSC_VER */
