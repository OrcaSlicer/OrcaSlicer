...
4. 层内多周回、**常规 rectilinear/平行线填充**、悬垂件、实心顶/底、支撑、多岛在**不补线**前提下皆不在适用范围内,判定器应直接拒绝而非尽力打印。
...
- **明确不可行**(直接拒绝):任意带**常规 rectilinear/断线填充**的层、多 wall 且环间不共点层、多岛层、支撑层、有顶盖实心层。若模型只有个别层违规,则"放弃该模式"整单回退常规打印(保守策略),或将来探索"违规层常规、其余层螺旋"的混合过渡(风险较高,列为开放问题)。
