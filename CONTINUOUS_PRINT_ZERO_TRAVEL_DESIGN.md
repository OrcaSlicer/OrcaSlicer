...
## 2026-09-19 启用条件补充：配置喷嘴数与使用材料数

“单材料”按任务分配的材料判断，不按打印机配置的喷嘴数量判断。双喷头机器可以在只使用一个材料时进入连续打印；闲置材料槽不影响启用。实现复用 `Print::extruders(true)`，将支撑/界面和显式工具切换一并纳入检查。单实例、逐层打印及禁用擦料塔的限制仍保留，层内可连接性仍单独检查。
