...
// Creality K2 SE G-code carries the producer spelled "Orca_Slicer" (see needs_underscored_producer_header()
// in GCode.cpp). Re-opening such a file must still find the header and import its CONFIG_BLOCK.
SCENARIO("G-code config loader accepts the Orca_Slicer producer alias", "[Config][Regression]") {
    GIVEN("G-code files whose header spells the producer with and without the underscore") {
        namespace fs = boost::filesystem;
        const fs::path dir = fs::temp_directory_path() / fs::unique_path("orca_alias_%%%%%%%%");
        fs::create_directories(dir);

        // load_from_gcode_file() rejects a CONFIG_BLOCK with fewer than 80 key/value pairs,
        // so dump a complete default config rather than a couple of hand-picked keys.
        auto full_config_block = [](const std::string &printer_model) {
            Slic3r::DynamicPrintConfig config = Slic3r::DynamicPrintConfig::full_print_config();
            config.set_key_value("printer_model", new Slic3r::ConfigOptionString(printer_model));
            std::string block = "; CONFIG_BLOCK_START\n";
            for (const std::string &key : config.keys())
                block += "; " + key + " = " + config.opt_serialize(key) + "\n";
            block += "; CONFIG_BLOCK_END\n";
            return block;
        };

        auto write_gcode = [&dir, &full_config_block](const std::string &name,
                                                       const std::string &producer) {
            const fs::path path = dir / name;
            boost::nowide::ofstream ofs(path.string());
            ofs << "; HEADER_BLOCK_START\n"
                << "; generated by " << producer << " 2.5.0 on 2026-07-25 at 12:00:00\n"
                << "; HEADER_BLOCK_END\n"
                << "G1 X10 Y10 E1\n"
                << full_config_block("Creality K2 SE");
            ofs.close();
            return path.string();
        };

        const std::string aliased  = write_gcode("aliased.gcode", "Orca_Slicer");
        const std::string standard = write_gcode("standard.gcode", "OrcaSlicer");
        const std::string foreign  = write_gcode("foreign.gcode", "SomeOtherSlicer");

        WHEN("the aliased file is loaded") {
            Slic3r::DynamicPrintConfig config;
            THEN("the header is accepted and the config block is imported") {
                REQUIRE_NOTHROW(config.load_from_gcode_file(aliased, Slic3r::ForwardCompatibilitySubstitutionRule::EnableSilent));
                REQUIRE(config.opt_float("layer_height") == Catch::Approx(0.2).margin(0.01));
                REQUIRE(config.opt<Slic3r::ConfigOptionString>("printer_model")->value == "Creality K2 SE");
            }
        }

        WHEN("the standard file is loaded") {
            Slic3r::DynamicPrintConfig config;
            THEN("the pre-existing spelling keeps working") {
                REQUIRE_NOTHROW(config.load_from_gcode_file(standard, Slic3r::ForwardCompatibilitySubstitutionRule::EnableSilent));
                REQUIRE(config.opt<Slic3r::ConfigOptionString>("printer_model")->value == "Creality K2 SE");
            }
        }

        WHEN("a file from an unrelated producer is loaded") {
            Slic3r::DynamicPrintConfig config;
            THEN("it is still rejected") {
                REQUIRE_THROWS_AS(config.load_from_gcode_file(foreign, Slic3r::ForwardCompatibilitySubstitutionRule::EnableSilent), Slic3r::RuntimeError);
            }
        }
