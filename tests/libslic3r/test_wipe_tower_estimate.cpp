...
static WipeTowerFootprint estimate(const ConfigBase &config, size_t count, double layer_height, double height, bool any_raft = false, WipeTowerType type = WipeTowerType::Type2)
{
    return estimate_wipe_tower_footprint(config, type, filaments(count), layer_height, height, any_raft);
}
...
TEST_CASE("Each planner spaces its purge lines by its own option", "[WipeTowerEstimate]") {
    ...
    const double type1_spaced = estimate(config, 3, 0.2, 5., false, WipeTowerType::Type1).depth;
    ...
    CHECK_THAT(estimate(config, 3, 0.2, 5., false, WipeTowerType::Type1).depth - 0.5, WithinAbs(1.5 * (type1_spaced - 0.5), 1e-6));
}
...
TEST_CASE("Type1 sizes the tower from each filament's own prime volume", "[WipeTowerEstimate]") {
    ...
    CHECK_THAT(estimate(config, 2, 0.21, 5., false, WipeTowerType::Type1).depth, WithinAbs(blocks, 1e-4));
    // The ids pick the volumes, so their order does not matter and a lone filament has no purge.
    CHECK_THAT(estimate_wipe_tower_footprint(config, WipeTowerType::Type1, {1, 0}, 0.21, 5., false).depth, WithinAbs(blocks, 1e-4));
    CHECK_THAT(estimate(config, 1, 0.21, 5., false, WipeTowerType::Type1).depth, WithinAbs(0., 1e-9));
    config.set_key_value("filament_adhesiveness_category", new ConfigOptionInts({0, 0}));
    CHECK_THAT(estimate(config, 2, 0.21, 5., false, WipeTowerType::Type1).depth, WithinAbs(11., 0.01));
    // A rib wall squares the same stack.
    config.set_deserialize_strict("wipe_tower_wall_type", "rib");
    const WipeTowerFootprint rib = estimate(config, 2, 0.21, 5., false, WipeTowerType::Type1);
...
TEST_CASE("A second nozzle adds the ramming of one nozzle change per layer", "[WipeTowerEstimate]") {
    ...
    const double same_nozzle = estimate(config, 2, 0.2, 5., false, WipeTowerType::Type1).depth;
    config.set_key_value("filament_map", new ConfigOptionInts({1, 2}));
    CHECK_THAT(estimate(config, 2, 0.2, 5., false, WipeTowerType::Type1).depth - same_nozzle, WithinAbs(3., 1e-4));
}
...
TEST_CASE("Object height sets the stability floor and the auto brim", "[WipeTowerEstimate]") {
    ...
    CHECK_THAT(estimate(config, 2, 0.2, 50., false, WipeTowerType::Type1).brim_width, WithinAbs(printed_brim(auto_brim, WipeTowerType::Type1), 1e-6));
}
...
TEST_CASE("A single filament only gets a tower when one is printed anyway", "[WipeTowerEstimate]") {
    ...
    CHECK_THAT(estimate(config, 1, 0.2, 100., false, WipeTowerType::Type1).depth, WithinAbs(WipeTower::get_wrapping_detection_depth(), 1e-9));
    config.set_key_value("enable_wrapping_detection", new ConfigOptionBool(false));
    ...
    CHECK_THAT(estimate(config, 1, 0.2, 100., true).depth, WithinAbs(20., 1e-9));
...
TEST_CASE("Every wall and tower type is read the same from a preset and a static config", "[WipeTowerEstimate]") {
    ...
    const WipeTowerFootprint fp          = estimate(preset, 3, 0.2, 5., false, type);
    const WipeTowerFootprint from_static = estimate(static_config, 3, 0.2, 5., false, type);
    CHECK(fp.depth > 0.);
    ...
    static_config.apply(preset, true);
    CHECK(estimate(preset, 1, 0.2, 5., false, type).depth > 0.);
    CHECK(estimate(static_config, 1, 0.2, 5., false, type).depth > 0.);
}
