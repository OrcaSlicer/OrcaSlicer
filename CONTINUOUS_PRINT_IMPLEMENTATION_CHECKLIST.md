...
## 2026-09-19 双喷头机器的单材料任务

用户的立方体与圆柱体工程采用 `MAGIC-HT-L 0.4 Nozzle - Dual`，配置两个 0.4mm 喷嘴，但模型只分配一个材料。旧启用条件直接检查 `nozzle_diameter.size() == 1`，导致两个工程在进入层规划前整体回退。

- 改用 `Print::extruders(true)` 检查任务所分配的材料，包含对象、支撑/界面及显式工具切换；未使用的喷嘴和材料槽不再导致拒绝。
- 保留单对象实例、逐层打印、禁用擦料塔及单材料限制；实际存在支撑或无法连接的层继续按既有逐层判定回退。
- 整体拒绝提示分别指出对象实例、打印顺序、擦料塔或材料数量的问题，删除已不适用的 Smooth Spiral 提示。
- 不修改连续打印的实心层、稀疏层及固定 Z 走线算法。回归用例见 `tests/manual/continuous_print_dual_tool.md`。
- Release 编译通过；用户圆柱体 129/129 层及仅用第二材料的副本 129/129 层通过零空驶、XY 连通、固定 Z 审计。用户立方体为 311/312 层，最顶层 `monotonicline` 回退；只在验证副本中改为 `monotonic` 后 312/312 层通过。
- 逐对象打印、多实例和两种实际材料的反例均产生准确拒绝原因；关闭功能时，除生成时间和对象标签 ID 外，G-code 与旧版完全一致。审计器 5 项测试通过。当前构建配置关闭了 C++ 测试，本次未运行 C++ 测试套件。
- 擦料塔反例的 CLI 配置被上游归一化为关闭，未覆盖擦料塔拒绝分支；该分支保留原检查条件，需用实际保留擦料塔的工程补测。新 DLL 已复制到 `build/OrcaSlicer/OrcaSlicer.dll`，旧 DLL 备份于验证目录。
