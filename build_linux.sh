...
# A slicer build needs the dependency tree. When the default tree (in deps/)
# is missing, build it first instead of failing later. Skipped under a dry run.
if [[ -n "${BUILD_ORCA}" ]] && [[ -z "${BUILD_DEPS}" ]] && [[ -z "${DRY_RUN}" ]] ; then
